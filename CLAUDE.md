...
7. **Derived-on-read invariants (OI-0117):** Fields that are derived from child rows at read time must not acquire stored columns on the parent. `events.date_in` / `events.time_in` were dropped in migration 028 because they duplicated the earliest child window. Two grep contracts keep this from regressing:
