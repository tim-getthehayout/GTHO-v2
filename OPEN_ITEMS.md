...
**Added:** 2026-04-20 | **Area:** v2-build / events / move-wizard / dmi-chart | **Priority:** P1 (live field-testing regression — every same-farm rotation produces a card whose 3-day chart is ⅔ blank until the new event has accrued 3 full days. First-day rotations render a single filled bar with two blank neighbors.)

**Status:** open — spec ready, implementation blocked on Tim's confirmation of scope (see decision point below)
...
**Always set `sourceEventId` on a new destination event in move-wizard.js:680**, regardless of same-farm vs cross-farm. Change line 680 from:
```js
sourceEventId: isCrossFarm ? sourceEvent.id : null,
```
to:
...
```
The "cross-farm" display logic at `src/features/events/index.js:335-360` (the "← Moved from {farm}" banner) should compare `sourceEvent.farmId !== evt.farmId` for the display decision, NOT use `sourceEventId` presence as the cross-farm flag. A quick read of that code block confirms the comparison is already present at line 358 (`e.sourceEventId === evt.id && e.farmId !== evt.farmId`) — the outgoing case uses farm comparison. But line 344 (`if (evt.sourceEventId)`) uses presence alone. That check needs to be tightened to `if (evt.sourceEventId && sourceEvt && sourceEvt.farmId !== evt.farmId)` so same-farm moves don't trigger the "← Moved from {farm}" banner.

**Data repair for existing events:** All pre-fix rotation events in Supabase have `source_event_id = NULL` even when a real source exists. Tim may want a one-time backfill. Scope TBD — see decision point.

**Decision point for Tim:**

1. **Minimum ship:** Fix `move-wizard.js:680` + `events/index.js:344` for forward-looking correctness. Existing events remain with null `source_event_id` — their charts stay blank for pre-start days. The user sees charts fill correctly starting on the next rotation.
2. **Minimum ship + backfill:** Additionally write a one-time script that walks the event graph (by operation + farm + date proximity) to infer and backfill `source_event_id` on existing rotations. Higher risk (graph reconstruction is heuristic for same-farm moves with gaps), higher value (Tim's existing charts heal immediately).

**Default recommendation:** Minimum ship (option 1). Existing events are historical data points; getting future rotations right is the P1. A backfill could be added later as a separate OI if the missing historical context bothers Tim on reflection.

**Files affected:**
- `src/features/events/move-wizard.js` — line 680 unconditional assignment
- `src/features/events/index.js` — line 344 add farm-id comparison to the incoming-move-from-other-farm banner guard
- `tests/unit/move-wizard.test.js` (if exists, else new) — assert same-farm move sets `sourceEventId` on the new event
- `tests/unit/dmi-chart-context.test.js` (or the cascade context test) — assert 3-day chart for a day-1 rotation event shows prior-event data for pre-start days
- E2E (new): full rotation on a single farm + verify `events.source_event_id` populated in Supabase; verify the new event's chart renders filled bars for the two pre-rotation days

**Acceptance criteria:**
- [ ] `move-wizard.js:680` sets `sourceEventId = sourceEvent.id` unconditionally for the `destType === 'new'` branch
- [ ] `events/index.js:344` display logic respects the farm-id comparison (same-farm moves don't trigger "← Moved from {farm}" banner even though `sourceEventId` is now populated)
- [ ] Unit test: same-farm move sets `sourceEventId` on the new event
- [ ] Unit test: cross-farm move still sets `sourceEventId` AND the display banner appears
...
- [ ] E2E test: full rotation → Supabase assertion on `source_event_id` + chart render
- [ ] Verify: create a new rotation in the live data, confirm E-3-style cards show filled bars for all 3 chart days instead of the current ⅔ blank
