...
**Checkpoint:** ship as its own spec-file handoff. Spec: `github/issues/derived-event-start-datetime.md`. Investigation-first, design-confirmation-second, implement-third — affects schema v28, CP-55/CP-56, every read site of `event.dateIn`, and the edit UIs for paddock + group windows. Not small surface. OI-0116 can ship independently and first; this spec converts OI-0116's target from the `events` column to the earliest child window when it lands, no user-visible change.

**Status:** open — spec ready for handoff.
...
**Open design question (flagged in spec for Tim to answer before implementation):** when the user moves event start LATER and multiple child windows are tied at the current earliest, does the write-through update (a) all tied windows to the new datetime, or (b) only the "primary" window (first-inserted paddock window) while leaving the others where they are? Option (a) matches the farmer's mental model ("move the start of the event"); option (b) preserves historical windows as immutable but makes the derived start not match the user's edit. **Spec recommends (a) with an on-save confirmation when more than one window moves.**
