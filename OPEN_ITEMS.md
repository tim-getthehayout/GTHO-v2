...
**Status:** partial — 2026-04-18. Bugs 1 / 2 / 4 / 6 shipped in earlier commit `e124952` (pre-session, never closed out): double unit suffix removed, acreage header "D  7.42 ac" present, badge logic for "stored feed & grazing" correct, `toLocaleString` comma formatting applied. Bugs 3 (green capacity line with est. AUDs / days remaining / ADA) and 5 (stored-feed DMI value) are wired structurally but render em-dashes pending DMI-8 / OI-0069 data flow. Bug 7 (top stat cards — Pasture %, NPK/Acre, NPK Value) renders em-dashes for the same upstream-data reason. Per the session brief's explicit carve-out, bugs 3 / 5 / 7 stay deferred until DMI-8 ships. Audited 2026-04-18 as part of the pre-testing-cleanup bundle; no new code shipped with the retroactive partial-close. GH-22 filed + closed with partial note. Spec file: `github/issues/GH-22_locations-tab-display-fixes.md`.
