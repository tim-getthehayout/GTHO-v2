...
> **STATUS: RETIRED 2026-05-04.** All sprint specs (SP-1 through SP-14) have been reconciled into the base design docs across Sessions A (schema), B (P1 UX), C (P2/P3 UX + architecture + infra + calc + design system), and D (SP-9 Survey Sheet + SUR-2 calc + SP-14 data-logic thin pointer). **Reconciliation backlog is fully closed.** This file is preserved as the historical record of how those base-doc sections originated. The Reconciliation Checklist below shows the per-SP destinations.
>
> **Active design specs now live in the base docs.** Read those when implementing — not this file:
> - V2_UX_FLOWS.md (UX flows, screen designs, dialogs — including §7 Survey Workflow with all 11 subsections)
> - V2_APP_ARCHITECTURE.md (snapshot/rollback, derive-on-read, store patterns)
> - V2_INFRASTRUCTURE.md (units, sync indicator, i18n, logging)
> - V2_CALCULATION_SPEC.md (formulas, helpers including `getLiveRemainingForMove` and SUR-2 `survey.baleRingCover`)
> - V2_DESIGN_SYSTEM.md (component patterns including `renderPreGrazeCard`)
> - V2_SCHEMA_DESIGN.md (table definitions, column drops, FK ordering)
>
> Do **not** edit this file going forward. If a sprint-spec section turns out to need correction, fix the base doc and add a Change Log row there.

Working design doc for the 2026-04-15 round of UI improvements. Accumulates all design decisions here first. At the end of the sprint, a dedicated reconciliation session merges finalized content into the base docs (V2_UX_FLOWS.md, V2_DESIGN_SYSTEM.md, etc.) and converts `github/issues/` files to thin pointers.

**Rule (during the sprint, now superseded):** Claude Code should read this file alongside the base docs during implementation. If this file conflicts with a base doc, this file wins (it's newer). — *Superseded 2026-05-04: base docs are now authoritative.*
...
|------|---------|-------------|
| 2026-05-04 | Reconciliation Checklist | **Reconciliation Session D executed — last open item closed; reconciliation backlog now fully complete.** Three checklist entries flipped: SP-9 (V2_UX_FLOWS.md §7 rewritten end-to-end into 11 subsections including the new §7.9 Surveys sub-tab on Locations Screen), `survey.baleRingCover` calc (added as SUR-2 in V2_CALCULATION_SPEC.md §4.9 — total formulas 39 → 40), and the SP-14 data-logic spec thin-pointer conversion (`github/issues/GH-34_dam-calf-bidirectional-sync.md` now points at the live `src/features/animals/calving-sync.js` helper module + `saveAnimal()` caller as the data-logic source of truth). One Change Log row each in V2_UX_FLOWS.md and V2_CALCULATION_SPEC.md. The retired-status block at the top of this file updated to remove the "one open follow-up" line — there are no open follow-ups now. No code changes — documentation catch-up only. Sprint reconciliation work complete. Owner: Cowork. |
| 2026-05-04 | Reconciliation Checklist | **Reconciliation Session C executed (RECONCILIATION_PLAN_2026-05-03 UX-5, UX-6, UX-7, UX-8, ARCH-1, ARCH-2, INFRA-1, INFRA-2, CALC-1, DS-1 + SP-14 bundled + thin-pointer sweep).** Ten reconciliation items landed across six base docs in one pass. **V2_UX_FLOWS.md:** new §3.4 (Empty Group Handling — UX-5/SP-11); §17.2 Header Bar Feedback & Help sub-row inlined from SP-6 reference (UX-6); new §21 Feedback Screen desktop-only (UX-7/SP-7); new §20.8 Forage Types Reference Library (UX-8/SP-13); §15.1 Animal Edit Sheet gained the Dam + Birth date shared-row spec (SP-14 bundle). **V2_APP_ARCHITECTURE.md:** new §4.5 Snapshot/Rollback Pattern (ARCH-1/SP-10 architecture portion + OI-0081); new §4.6 Derive on Read, Don't Store doctrine (ARCH-2 — lifts OI-0117/OI-0133/OI-0139/OI-0141 lessons out of CLAUDE.md grep contracts into the architecture doc). **V2_INFRASTRUCTURE.md:** new §1.4 Unit Families with the seven-family table including the new `dmYieldDensity` (INFRA-1/SP-13); new §9 Sync Indicator (INFRA-2/OI-0141 honest two-axis state). **V2_CALCULATION_SPEC.md:** new `getLiveRemainingForMove` helper documentation in §4.6 (CALC-1/OI-0139 — strict-> rule, same-instant edge case, three current consumers). **V2_DESIGN_SYSTEM.md:** new §3.16 Pre-Graze Card shared component (DS-1/OI-0112 successor to OI-0100). **github/issues/ thin-pointer sweep:** seven SP-N spec files converted to thin pointers (`empty-group-archive-flow.md`, `feedback-help-header-buttons.md`, `feedback-screen-desktop.md`, `field-mode-v1-parity.md`, `GH-11_dashboard-card-enrichment.md`, `GH-29_dmi-8-cascade-rewrite.md`, `GH-33_forage-types-settings-ui.md`). Reconciliation Checklist entries flipped: SP-6 ✅, SP-7 ✅, SP-10 architecture ✅, SP-11 UX ✅, SP-13 ✅, SP-14 ✅, thin-pointer sweep ✅ (partial — SP-9 deferred to Session D). **Deferred to Session D:** SP-9 (survey sheet v1 parity merge into V2_UX_FLOWS.md §7 — bigger; not in original Session C scope), and the SP-9 + SP-14 data-logic spec thin-pointer conversions. No code changes — documentation catch-up only. Sprint spec retirement decision in this session's commit. |
| 2026-05-03 | Reconciliation Checklist | **Reconciliation Session B executed (RECONCILIATION_PLAN_2026-05-03 UX-1, UX-2, UX-3, UX-4).** Four base-doc edits to V2_UX_FLOWS.md flipped to ✅ in the checklist above: SP-3 (§17.7 Dashboard location card v1-parity rewrite — 15-element anatomy, two deliberate deltas, OI-0065/OI-0066 callouts), SP-8 (§16 Field Mode v1-parity rewrite — header pill activation, body.field-mode CSS gate, 8-module tile grid, expandable cards reusing buildLocationCard, 2-step Heat picker, exit-returns-to-previous via sessionStorage), SP-12 (§17.15 DMI chart 5-state status model + deficit segment + sub-move-close forced feed-check rule), and SP-10 UX (new §17.15.1 "Event Data Editing" — gap/overlap resolver, retro-place atomic two-write, §7 Groups Edit dialog, §12 Sub-moves Edit dialog, event-level dates + Event Reopen, §8 Feed Entries validation, §8a Move Feed Out 4-step flow, §9 Feed Checks invariant + Re-snap, §3/§6 inline observation edit). Session A pattern followed (one Change Log row per session in V2_UX_FLOWS.md and one inaccuracy correction in RECONCILIATION_PLAN). Sessions C still owes SP-6, SP-7, SP-9, SP-11 UX, SP-13, SP-14, SP-10 architecture (ARCH-1 / ARCH-2 in V2_APP_ARCHITECTURE.md), SP-10 calc updates (CALC-1), INFRA-1, INFRA-2, DS-1, plus the github/issues/ thin-pointer sweep. |
| 2026-05-03 | Reconciliation Checklist | **Reconciliation Session A executed (RECONCILIATION_PLAN_2026-05-03 SCH-1, SCH-2, SCH-3).** Schema portions of SP-10 (event_feed_entries new columns + check constraints), SP-11 (groups archived → archived_at), and the bale-ring rename verification all flipped to ✅ in the checklist above. UX, calc-spec, and CP-55/CP-56 portions remain pending; UX/architecture work scheduled for Sessions B/C, CP-55/CP-56 follow-up captured as **OI-0156** (single sweep across all four columns landing in this reconciliation pass). Sprint spec is not yet retired — Sessions B and C still owe SP-3, SP-6, SP-7, SP-8, SP-10 UX, SP-11 UX, SP-12 UX, SP-13, SP-14 base-doc merges. |
...
**Status:** Shipped 2026-04-20 (commits ca332c0 spec → 65fc3b8 implementation; GH-29 closed). All five chart statuses + cascade allocation + forced feed-check on sub-move close + date-routing source-event bridge live in production. Suite 1135 → 1165 (+30). Reconciliation into base docs still pending — see checklist below.
...
- [x] **Merge SP-3 card spec into V2_UX_FLOWS.md §17.7** (replace current minimal card body spec) — **done 2026-05-03 (RECONCILIATION_PLAN_2026-05-03 UX-1, Session B).** §17.7 rewritten to v1 parity: 15-element card anatomy (left green accent bar through DMI/NPK summary), explicit "Deliberate v1 deltas" section (small bottom Feed/Feed-check buttons removed; large green Feed button added under large amber Feed check), "What is NOT on this card" callouts for OI-0065 (per-group reweigh moved to Animals) and OI-0066 (per-group Move event-scoped). Action buttons documented inline (Edit opens §17.15 sheet, Move all opens move wizard). Dashboard `Survey` button confirmed dropped (not in v1 either). `github/issues/dashboard-card-enrichment.md` referenced as canonical implementation source.
- [x] **Verify §17.15 (already in base doc) is still accurate after implementation** — verified during Session B (RECONCILIATION_PLAN_2026-05-03 UX-3). DMI chart subsection now documents the 5-state status model (actual / estimated / needs_check / no_animals / no_pasture_data) with deficit segment, "Feed check needed" hint, partial pre-graze "(Fix)" hint, source-event date-routing-only bridge, and the forced feed-check on sub-move close rule. Other §17.15 sections (Location / Event Summary / Pre-graze / Groups / Feed Entries / Feed Checks / DMI breakdown / Notes / Sub-move history / Actions) remain as previously written — no drift detected.
- [x] **Verify §17.7 action buttons (already in base doc) match final implementation** — verified during UX-1 (Session B). Action buttons documented in the new §17.7 anatomy (Move calls openMoveWizard directly; Edit opens openEventDetailSheet via the SP-2 sheet pattern). Old "Survey" button on dashboard confirmed dropped per SP-3 spec (v1 didn't have it either; survey creation lives on §7 Surveys + per-paddock CTA in §17.15).
- [x] **Convert `github/issues/` files to thin pointers referencing base doc sections — done 2026-05-04 (Session C, partial).** Seven files converted in this pass: `empty-group-archive-flow.md` → V2_UX_FLOWS.md §3.4; `feedback-help-header-buttons.md` → V2_UX_FLOWS.md §17.2; `feedback-screen-desktop.md` → V2_UX_FLOWS.md §21; `field-mode-v1-parity.md` → V2_UX_FLOWS.md §16; `GH-11_dashboard-card-enrichment.md` → V2_UX_FLOWS.md §17.7; `GH-29_dmi-8-cascade-rewrite.md` → V2_CALCULATION_SPEC.md §4.2 + V2_UX_FLOWS.md §17.15; `GH-33_forage-types-settings-ui.md` → V2_UX_FLOWS.md §20.8 + V2_INFRASTRUCTURE.md §1.4. SP-9 (`GH-12_survey-sheet-v1-parity.md`) remains pending until SP-9's UX merge lands (deferred to Session D).
- [ ] Update V2_BUILD_INDEX.md with completed work
- [x] **Merge SP-6 feedback/help buttons into V2_UX_FLOWS.md §17.2 (add sub-row to header spec) — done 2026-05-04 (Session C, UX-6).** §17.2 "Header Bar" "Feedback & Help sub-row" subsection now carries the full inlined spec — sub-row layout, button styling, responsive behavior, Feedback vs Get Help sheet differences (categories, Priority dropdown), shared structure (auto-filled context tag + Area dropdown using v2 screen names), and the no-FAB-in-v2 statement. `github/issues/feedback-help-header-buttons.md` converted to thin pointer.
- [x] **Merge SP-7 feedback screen into V2_UX_FLOWS.md as new §21 — done 2026-05-04 (Session C, UX-7).** New §21 "Feedback Screen (Desktop-Only)" documents the desktop-sidebar-only nav placement (between Settings and the sync strip; never in mobile bottom nav) with red unread badge for `open` + `resolved` items; four screen sections matching v1's order (Confirmation banner + cards; Stats strip; Dev session brief card; All Submissions card with type/area/status filters); resolve/edit on detail sheet only; mobile fallback (centered "desktop-only" card); out-of-scope items. Zero schema impact. `github/issues/feedback-screen-desktop.md` converted to thin pointer.
- [x] **Merge SP-8 field mode into V2_UX_FLOWS.md §16** (replace current field mode spec with v1 parity version; include "exit returns to previous screen" behavior) — **done 2026-05-03 (RECONCILIATION_PLAN_2026-05-03 UX-2, Session B).** §16 rewritten to 12 subsections covering: header pill activation with three-state context-aware behavior (⊞ Field / ← Detail / ⌂ Home), exit-returns-to-previous via sessionStorage, body.field-mode CSS gate (sidebar / bottom nav / SP-6 sub-row / build stamp hidden + desktop grid collapse), 8-module tile grid driven by `FIELD_MODULES` with 4-module default, shared event picker sheet (Move / Feed Check / Heat fallback), expandable event cards reusing `buildLocationCard()`, interactive tasks with checkboxes + due-date color coding, 2-step Heat picker (event/group filter pills, search, multi-record), feed-loop behavior on Feed Animals tile, field-mode sheet treatment (no backdrop close, hidden handle, "⌂ Done", full-screen mobile, after-save→#/field), Module Settings card in Settings (§20 reference). The v2 dark-green field-mode header bar is explicitly deleted. Spec file `github/issues/field-mode-v1-parity.md` referenced as canonical implementation source.
- [x] **Merge SP-9 survey sheet v1 parity into V2_UX_FLOWS.md §7 — done 2026-05-04 (Session D).** §7 rewritten end-to-end into 11 subsections covering: §7.1 three modes (one sheet — `bulk` / `single` / `bulk-edit` — toggled by `setSurveySheetMode`); §7.2 the 8-path entry-point matrix (v1's home Pasture readiness card explicitly dropped); §7.3 paddock card with all six body sections, recovery-window date math inverse on commit; §7.4 the bale-ring residue helper (the one deliberate v2 addition); §7.5 bulk-mode chrome (action row, date row, farm/type pills, search, in-place finish-confirm bar); §7.6 draft lifecycle (immediate localStorage + 1s debounced Supabase, three close semantics); §7.7 commit rules (require ≥1 rated paddock, one observation per paddock with `source='survey'` / `confidenceRank=3`, recovery-window inversion on save, bulk-edit replaces); §7.8 field-mode adaptations (backdrop disabled, handle hidden, `⌂ Done`, picker sheet for single in field); **§7.9 Surveys sub-tab on Locations Screen** (the v1 surface — draft banner + committed list); §7.10 design notes; §7.11 schema and export impact. `github/issues/GH-12_survey-sheet-v1-parity.md` converted to thin pointer.
- [x] **Document the `survey.baleRingCover` calc in V2_CALCULATION_SPEC.md — done 2026-05-04 (Session D).** New SUR-2 added to §4.9 Survey/Forage Quality Domain. Documents the four-input → four-output shape, the rings-as-bare-ground model with note on direction-flip flexibility, the paddock-area fallback, and the storage rule (count on observation, derived cover only flows in via auto-fill and is then persisted as a normal `forage_cover_pct`). Total formulas: 39 → 40.
- [x] ~~Update V2_SCHEMA_DESIGN.md §4 (farm_settings) to include `bale_ring_residue_diameter_cm`~~ — **done 2026-04-18 in §1.3 farm_settings, plus 2026-04-18 Change Log row.** `event_observations` was dropped entirely (migration 029, OI-0113); §5.8 documents the removal — so `bale_ring_residue_count` is no longer a live column to track.
- [ ] Update CP-55 / CP-56 spec(s) with new farm_settings column handling and schema_version bump to 22. *(Subsumed by OI-0156 — CP-55/CP-56 catch-up sweep.)*
- [x] **SP-10 reconciliation (UX portion) — UX-4 done 2026-05-03 (RECONCILIATION_PLAN_2026-05-03 UX-4, Session B).** New V2_UX_FLOWS.md §17.15.1 "Event Data Editing" created covering: core principle (compute on read for derived; explicit reconciliation for structural), shared gap/overlap routine (3 options each), retro-place atomic two-write flow (no reopen ceremony), §7 Groups Edit dialog, §12 Sub-moves Edit dialog (resolves OI-0064), event-level dates (`date_in` direct edit + Event Reopen for `date_out`), §8 Feed Entries validation, §8a Move Feed Out (4-step sheet, two entry points, atomic transaction, DMI logic block), §9 Feed Checks invariant + Re-snap dialog, §3 Pre-graze + §6 Post-graze inline edit + silent cascade, sub-move close forced feed-check rule. **Architecture portion (ARCH-1: Snapshot / Rollback subsection in V2_APP_ARCHITECTURE.md) — done 2026-05-04 (Session C, ARCH-1).** New V2_APP_ARCHITECTURE.md §4.5 "Snapshot / Rollback Pattern (multi-step edit flows)" documents the SP-10 Phase 1 helper (`src/data/snapshot.js`), the four primitives (`takeSnapshot`/`stagedRead`/`commit`/`reSnap`), the two failure modes the pattern prevents (cancel-after-partial-commit and re-render cascade clobbering staged edits per OI-0115), gap/overlap resolver re-snap behavior, atomic two-write transactions as commit-time fan-outs, and the grep contract barring direct `store.add/update/remove` in multi-step edit flows before final confirm. **`getLiveRemainingForMove` calc helper (CALC-1) — done 2026-05-04 (Session C, CALC-1).** SP-10 spec file thin-pointer conversion: not applicable — SP-10 never had its own `github/issues/` spec file (the SP-10 walk-through was authored directly into UI_SPRINT_SPEC.md), so nothing to convert.
- [x] **SP-10 §8a schema reconciliation** — ~~update V2_SCHEMA_DESIGN.md `event_feed_entries` table to include `entry_type`, `destination_type`, `destination_event_id` columns + check constraints~~ **done 2026-05-03 (RECONCILIATION_PLAN_2026-05-03 SCH-3).** Calc spec updates (DMI-1, DMI-5, NPK-1, NPK-2, cost-per-day to reflect "sum deliveries minus removals") **pending** — Session C. CP-55/CP-56 spec updates **subsumed by OI-0156**. V2_MIGRATION_PLAN.md §5.3a ordering verified — no change needed, the new FK points at the same `events` table it already references.
- [x] **SP-11 schema reconciliation** — ~~replace `archived boolean` with `archived_at timestamptz` in §3.3~~ **done 2026-05-03 (RECONCILIATION_PLAN_2026-05-03 SCH-1).** **UX portion done 2026-05-04 (Session C, UX-5):** new V2_UX_FLOWS.md §3.4 "Empty Group Handling (Archive Cascade)" documents the post-window-split trigger (`maybeShowEmptyGroupPrompt(groupId)` — no centralized cascade), three group states (`active` / `empty-but-active` / `archived`) on the `groups.archived_at TIMESTAMPTZ` column, the empty-group prompt (Archive primary / Keep active / Delete with hard-disable when group has any historical event_group_window), the Group Management UI ("Show archived" toggle + per-row Reactivate + delete-history guard), the picker filter list, and the CP-55/CP-56 OI-0156 catch-up note. V2_MIGRATION_PLAN.md §5.3a — no FK change needed. CP-55/CP-56 spec updates with `archived_at` column handling **subsumed by OI-0156**. `github/issues/empty-group-archive-flow.md` thin-pointer conversion **done 2026-05-04 (Session C)**.
- [x] **SP-13 reconciliation (Settings > Forage Types access, OI-0125) — done 2026-05-04 (Session C, UX-8 + INFRA-1).** New V2_UX_FLOWS.md §20.8 "Forage Types (Reference Library)" documents the Settings card (between Farm Settings and Field Mode), card anatomy, Add/Edit sheet fields with the unit-aware descriptor pattern, store-call signatures with full sync params, delete hard-guard against in-use forage types, empty-state with "seed defaults" link, out-of-scope items (archive UI, custom unit labels, forage quality grading), and zero schema/CP-55/CP-56 impact. V2_INFRASTRUCTURE.md §1.4 "Unit Families" documents the new `dmYieldDensity` family (stored kg/cm/ha, displayed lbs/in/ac, single canonical constant `DM_LBS_IN_AC_TO_KG_CM_HA` from `src/data/v1-migration.js`) alongside the seven-family table and a six-step "adding a new family" checklist. V2_SCHEMA_DESIGN.md §2.2 verified as still accurate — no column change. `github/issues/GH-33_forage-types-settings-ui.md` converted to thin pointer.
- [x] **SP-12 reconciliation (DMI-8 cascade chart, OI-0119) — done 2026-05-03 (RECONCILIATION_PLAN_2026-05-03 UX-3, Session B).** V2_UX_FLOWS.md §17.7 (dashboard card chart element #11) updated implicitly via the SP-3 v1-parity rewrite, which references §17.15.1 for the chart status model. V2_UX_FLOWS.md §17.15 DMI chart subsection now documents the full 5-state status enumeration table (actual / estimated / estimated-with-deficit / needs_check / no_pasture_data with both reasons / no_animals), deficit segment render, conditional legend, partial pre-graze "(Fix)" hint, source-event date-routing-only bridge, and the forced feed-check on sub-move close rule. V2_UX_FLOWS.md §12 Sub-moves rule cross-referenced from §17.15.1's "Sub-move Close — forced feed check" subsection. V2_CALCULATION_SPEC.md §4.2 DMI-8 was already authoritative (rewritten inline with OI-0119); the UX flow doc now points to it. **`github/issues/GH-29_dmi-8-cascade-rewrite.md` thin-pointer conversion pending — Session C** (sweep with the other SP-N spec files).
- [x] **SP-14 reconciliation (Edit Animal Dam + Birth date row, OI-0132) — done 2026-05-04 (Session C, bundled).** V2_UX_FLOWS.md §15.1 "Animal Edit Sheet" gained a "Dam + Birth date — shared row layout (SP-14, OI-0132)" subsection covering: flex-row two-column layout (~55–60% Dam, ~35–40% Birth date), removal of the separate full-width Birth date row that previously rendered after Notes, dynamic Birth date label hint that toggles `optional` (grey, `--text2`) ↔ `required` (red, `--red`) on Dam selection via a `change` listener, initial render of an existing calf-with-dam showing `required` immediately, inline error below the Birth date field on the OI-0132 Class A hard-gate trip ("Birth date is required when a dam is set."), narrow-mobile wrap fallback (≤ 400px), and the cross-reference to the data-logic side that shipped 2026-04-22 in commit `e9b40eb` at `src/features/animals/calving-sync.js`. **`github/issues/GH-34_dam-calf-bidirectional-sync.md` thin-pointer conversion done 2026-05-04 (Session D)** — points at the live helper module (`src/features/animals/calving-sync.js`) and `saveAnimal()` as the data-logic source of truth, and at V2_UX_FLOWS.md §15.1 for the UX layout the helper supports.
