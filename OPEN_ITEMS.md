...
**Added:** 2026-05-01 | **Area:** v2-build / feed / ui / data-integrity | **Priority:** P0 (live field-data corruption — Tim's G-event hit this today: a bale physically placed at G-1 was silently recorded at G-3 because G-3 happened to be `activePWs[0]` in the unsorted localStorage array; same code path silently misrouted a separate event's bale on Apr 30; healed via direct SQL this session, root cause still unfixed) | **Status:** closed 2026-05-01 — landed in commit `{HASH}` (GH-39). Delivery sheet now sorts open paddock windows most-recently-opened first; renders a `<select>` picker only on multi-window events; default = top option; chip `→ {locationName}` always rendered (single-window auto-pick visible too); save loop reads the mutable `selectedLocationId` ref (not the unsorted-localStorage shortcut). `→ {locationName}` chip also added to every §8 Feed Entries row in event detail and every line header in the feed-check sheet. 1300/1300 tests passing including 11 new (5 picker + 3 row-display + 3 line-display) + 1 new e2e Supabase round-trip. `grep -n "activePWs\[0\]" src/features/feed/` returns 0 matches; CLAUDE.md §"Architecture Audit" §6 extended with "Multi-window paddock disambiguation" sub-bullet.
