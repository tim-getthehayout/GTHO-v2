...
**Phase:** 3 — Build
**Active work:** Phase 3.3 Assessment — CP-24 through CP-33 in progress. Phase 3.2 backfill done (OI-0004/5/6/7 closed). animal_notes schema amendment applied. 550 tests passing. Field mode moved from Phase 3.5 into Phase 3.3 as CP-34 (all CPs after CP-33 renumbered +1; Phase 3.4 and 3.5 renumbered accordingly).
**Next up:** CP-34 (Field mode).
...
| CP | What to build | Spec source | Acceptance criteria |
|----|--------------|-------------|---------------------|
| CP-54 | Rotation calendar | V2_DESIGN_SYSTEM.md §4.6, v1 Reports screen | Paddock rows × month columns. Colored cells for grazing duration (AUDS·days). Active event pulse animation. Season totals sidebar. Desktop-only (hide on mobile per v1 pattern). Legend: pasture grazing, sub-move, active now. |
