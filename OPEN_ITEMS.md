...
**Status:** closed 2026-06-23 — guided dialog shipped on top of the OI-0185 helper. New `src/features/events/date-conflict-dialog.js` exports `openDateConflictDialog({ conflicts, operationId, onResume, event })`. The dialog renders one row per conflict with structured copy ("E-5 opens 2026-06-14, after your 2026-06-06") and three buttons: `Set open date to {outDate}` (clamps the conflicting window's `dateOpened`/`dateJoined` via the existing entity `update()` path so the OI-0184 sync queue picks it up, then closes the dialog and fires `onResume`), `Edit…` (jumps to `openEditPaddockWindowDialog` for paddock-kind conflicts, reusing OI-0064), and a top-level `Cancel`. When the list has > 1 entry, a `Fix all` footer button clamps every open date in one click. `close.js` `executeClose` and `move-wizard.js` `executeMoveWizard` now hand the OI-0185 conflict list to the dialog with `onResume` closures that re-enter the same execute function — the re-entered call re-runs the OI-0185 pre-flight, so a partial fix that left a residual conflict simply re-opens the dialog rather than slipping through. Z-index 230 sits above the close-event sheet (210), so the user can keep their typed dateOut/timeOut intact during the fix. Tests: 7 in `tests/unit/date-conflict-dialog.test.js` — paddock-line rendering (no raw validator string anywhere in `document.body.textContent`), Fix-all only when > 1 conflict, Set-open-date clamps + fires `onResume`, Fix-all clamps all + fires `onResume` once, Cancel writes nothing and skips `onResume`, Edit… opens `openEditPaddockWindowDialog`, and the end-to-end happy path (close.js → conflict → dialog → set-open-date → resume → successful close with all writes landed). Plus the OI-0185 integration tests were upgraded to assert the dialog opens (`[data-testid="date-conflict-dialog-title"]`) and that the raw `dateClosed must be on or after` string is nowhere in `document.body.textContent`. Grep contract holds: `grep -rn "dateClosed must be on or after" src/features/events/` returns 0 — the string lives only in the entity validators (`src/entities/event-paddock-window.js:47` + `src/entities/event-group-window.js:44`), as the spec requires. i18n keys consumed (added in the OI-0185 ship): `event.windowCloseConflictTitle`, `event.windowCloseConflictParagraph`, `event.windowCloseConflictPaddockLine`, `event.windowCloseConflictGroupLine`, `event.windowCloseConflictSetOpenDate`, `event.windowCloseConflictEdit`, `event.windowCloseConflictCancel`, `event.windowCloseConflictFixAll`. Suite: 1550 → 1557 (+7), all green; lint clean (0 errors). V2_UX_FLOWS.md §9.3 already documented the guided-correction step at filing time. Filed as GH-56.

**Bug.** When a close/move hits a window-date conflict, the user sees the raw entity message "Validation failed for eventPaddockWindows: dateClosed must be on or after dateOpened" (surfaced by `executeClose`'s catch in `src/features/events/close.js` and the move-wizard save-error toast). It names no paddock, gives no date, and offers no fix. The fix path exists — `openEditPaddockWindowDialog` (`src/features/events/edit-paddock-window.js`) can correct a window's open date — but it is undiscoverable from the error. Result: the user is wedged on a recoverable problem.

**Fix (design).** Consume the OI-0185 pre-flight conflict list and render a plain, specific dialog instead of throwing/toasting raw text. Example copy: *"E-5 is recorded as opening Jun 14, which is after your close date of Jun 6. Cattle can't leave a paddock before they arrived."* with one-tap actions:

- **Set E-5's open date to [close date]** — applies the correction and resumes the original close/move in one flow. Not silent — the date is shown and the user taps it.
- **Edit E-5…** — opens `openEditPaddockWindowDialog` for full control.
- **Cancel.**

When more than one window conflicts, list them all; a **Fix all** action clamps each conflicting open date to the out-datetime.

**Files:** new `src/features/events/date-conflict-dialog.js` (renders the conflict list from the OI-0185 helper, wires the one-tap fix + the edit-dialog jump), `src/features/events/close.js` + `src/features/events/move-wizard.js` (call the dialog on pre-flight failure instead of surfacing raw text), `src/i18n/locales/en.json` (conflict copy), tests.

**Acceptance criteria:**

- [ ] A close/move date conflict opens the guided dialog naming each conflicting paddock and its open date vs. the chosen out-date — the raw "dateClosed must be on or after dateOpened" string is never shown to the user.
- [ ] "Set open date to [out-date]" corrects the window and resumes the original close/move to success in one flow.
- [ ] "Edit…" opens `openEditPaddockWindowDialog` for that window.
- [ ] Multi-conflict: each conflicting window is listed; "Fix all" clamps each open date to the out-date.
- [ ] No silent data changes — every correction is shown and tapped.
- [ ] Grep contract: `grep -rn "dateClosed must be on or after" src/features/events/` returns 0 user-facing surfacings (the string stays only in the entity validator).
- [ ] V2_UX_FLOWS.md close/move flow documents the guided-correction step.

**CP-55/CP-56 spec impact:** None — UX/copy plus reuse of the existing edit path; no schema, column, JSONB, or migration change.

**Related:** OI-0185 (provides the pre-flight conflict list this renders — build 0185 first). OI-0162 (move-wizard error-toast plumbing this replaces for the date-conflict case). OI-0064 (the paddock-window edit dialog reused here).

**Origin:** 2026-06-23 cowork session; Tim got wedged closing the E-5 event with no in-app guidance to the fix.

---

### OI-0185 — Close-event and move-wizard are non-atomic on window-date conflicts: a failed close/move can leave group windows closed with no destination, orphaning animals (extends OI-0162)

**Added:** 2026-06-23 | **Area:** v2-build / events / close / move-wizard / data-integrity | **Priority:** P0 (live repro 2026-06-14 — a move E-series→D threw on a source paddock-window close and left 46 head unplaced; the same conflict still blocks Close Event today)

**Status:** closed 2026-06-23 — pre-flight + atomic apply shipped on both paths. New shared helper `src/features/events/window-close-guard.js` exports `checkWindowCloses(eventId, outDate, outTime, opts)` (dry-runs every open child window through the OI-0137 entity validators and returns a structured conflict list with `kind`, `windowId`, `eventId`, `groupId`/`locationId`, `openDate`, `outDate`, and the raw `entityError` for logs/tests only) and `hasWindowCloseConflict` convenience. Opts cover the move-wizard's scoped-mode (`scopedGroupWindowIds`) and the move-wizard's PW-only-when-last-group-leaves predicate (`includePaddockWindows`). `src/features/events/close.js` `executeClose` calls the helper after the overlap-warning confirm and before any write — non-empty conflict list surfaces `event.windowCloseConflict` in `statusEl` and returns with zero writes (event date_out unchanged, no PW close, no GW left-stamp). `src/features/events/move-wizard.js` `executeMoveWizard` calls the helper after the OI-0162-C idempotency guards and before the close branch — non-empty list surfaces the same i18n message and returns with zero source-departure writes, no orphan destination, no half-closed PW. Pre-flight scope: scoped move passes the single GW id; full-event move passes null; `willCloseSourcePaddocks` mirrors the existing `lastGroupLeaving` predicate so PW conflicts only flag when PW closes will actually fire. i18n keys added (used by 0185 statusEl surface and to seed OI-0186's dialog copy): `event.windowCloseConflict`, `event.windowCloseConflictTitle`, `event.windowCloseConflictParagraph`, `event.windowCloseConflictPaddockLine`, `event.windowCloseConflictGroupLine`, `event.windowCloseConflictSetOpenDate`, `event.windowCloseConflictEdit`, `event.windowCloseConflictCancel`, `event.windowCloseConflictFixAll`. 13 new tests: 10 in `tests/unit/window-close-guard.test.js` (helper unit — no/clean/PW/GW/scoped/skip/defensive paths plus the 2026-06-14 repro shape), 3 in `tests/unit/close-move-window-preflight.test.js` (close.js abort-with-zero-writes; close.js clean-path no-regression; move-wizard.js 2026-06-14 E-series→D regression — no GW left-stamp, no orphan dest event). Grep contract holds: `grep -rn "window-close-guard\|checkWindowCloses" src/features/events/` shows both `close.js:15` (import) + `close.js:186` (call), and `move-wizard.js:24` (import) + `move-wizard.js:544` (call), plus the helper's own definition. Raw "dateClosed must be on or after" / "dateLeft must be on or after" strings are never user-facing — both surfaces render `event.windowCloseConflict` instead, and the integration tests assert that. Existing OI-0162 tests (move-wizard-three-bugs.test.js — Bug A skip-zero, Bug B finally-close, Bug C idempotency) and the 0162 D→B-3 live-repro regression all pass unchanged. Suite: 1537 → 1550 (+13), all green. V2_UX_FLOWS.md §9.2 already documented the pre-flight contract at filing time. Filed as GH-55.

**Bug.** When closing an event (`src/features/events/close.js` `executeClose`) or moving (`src/features/events/move-wizard.js` `executeMoveWizard`), the code closes every open child window to the chosen out-datetime. If any window's open date is LATER than that out-datetime, the entity validator throws (`event-paddock-window.js`: "dateClosed must be on or after dateOpened"; `event-group-window.js`: "dateLeft must be on or after dateJoined"). Neither path validates window-close legality up front, so the throw lands mid-sequence:

- `executeClose` has **no pre-flight and no rollback** — it writes feed checks, then loops `closePaddockWindow`, then `closeGroupWindow`, then sets `events.date_out`. A throw in the loop leaves a partial close (some windows closed, event not finished) and surfaces the raw `err.message`.
- `executeMoveWizard` (post-OI-0162) wraps Steps 1–8 in try/catch/finally and pre-flights *FeedEntry* creation + idempotency, but it does **not** pre-flight the source window close dates. So a move whose source has a window opening after the move-out datetime still writes the group departures, then throws on the paddock-window close.

**Live repro.** `app_logs` 2026-06-14 10:16:35 — `source: move-wizard`, message "Validation failed for eventPaddockWindows: dateClosed must be on or after dateOpened", context `locationId = a334f135` (D), `sourceEventId = 68ff47f6` (E-series). E-5's source window opened 2026-06-14 (entry-date default) while the move-out was 2026-06-06; the move stamped the four group windows `left 06-06`, never created the D destination window, and orphaned 46 head (Cow-Calf Herd 27, Replacement Heifers 9, Shenk Cows 4, Shenk Yearling Heifers 6). Repaired by direct SQL this session.

**Fix (design).** Validate the whole operation before any write, and make it all-or-nothing:

1. **Pre-flight window-close validation** in both `executeClose` and `executeMoveWizard`: before writing anything, dry-run every open source window's proposed close (`dateClosed = out` for paddock windows, `dateLeft = out` for group windows) through its entity `validate()`. If any fail, abort with zero writes and hand the conflict list to the OI-0186 guided-correction dialog (no raw validator text).
2. **Atomic apply.** Only after pre-flight passes do the writes run. Order writes so the destination/arrival is created and validated before source departures are stamped, so a failed move can never strip placement without re-placing. (If a true staged-commit/rollback is feasible in the store, prefer it; at minimum the ordering guarantee + pre-flight eliminate the orphan path.)

**Files:** `src/features/events/close.js` (`executeClose` pre-flight + atomic ordering), `src/features/events/move-wizard.js` (`executeMoveWizard` source-window pre-flight, extending OI-0162's pre-flight block), new shared helper `src/features/events/window-close-guard.js` (dry-run-close validation returning the conflict list; shared by close + move + the OI-0186 dialog), `src/i18n/locales/en.json`, tests.

**Acceptance criteria:**

- [ ] Closing an event where an open window opens after the chosen close datetime makes **zero writes** (no half-closed windows, event `date_out` unchanged) and returns the conflict list.
- [ ] A move whose source window opens after the move-out datetime makes **zero source-departure writes** when the pre-flight fails — reproduce the 2026-06-14 E-series→D case and assert no group window is stamped `left` and no orphan destination is created.
- [ ] On a clean close/move, behavior is unchanged (regression: all existing `close` + `move-wizard` tests pass).
- [ ] Grep contract: the shared dry-run-close helper is the single validation path — `grep -rn "window-close-guard\|checkWindowCloses" src/features/events/` shows `close.js` and `move-wizard.js` as consumers.
- [ ] Unit tests: partial-state-on-throw is impossible for both paths (synthetic conflicting window → assert store unchanged).
- [ ] V2_UX_FLOWS.md close/move flow documents the pre-flight + all-or-nothing contract.

**CP-55/CP-56 spec impact:** None — control-flow + validation ordering only; no schema/column/JSONB/migration change.

**Related:** OI-0162 (move-wizard partial-write/idempotency — this extends it to the window-close-date dimension and to `close.js`, which OI-0162 did not touch). OI-0186 (guided correction — consumes this pre-flight's conflict list). OI-0137 (the `dateClosed >= dateOpened` / `dateLeft >= dateJoined` validators this pre-flights). OI-0184 (sibling reliability fix from the same session).

**Origin:** 2026-06-23 cowork debugging of Tim's unplaced-herd incident; root-caused to the 2026-06-14 move-wizard throw leaving a partial close.

---

### OI-0184 — Failed syncs vanish into a silent dead-letter queue that is never surfaced or auto-retried — local corrections can permanently fail to reach Supabase while the app looks "synced"

**Added:** 2026-06-23 | **Area:** v2-build / data / sync / data-integrity / observability | **Priority:** P0 (silent data-divergence — a saved correction can never reach Supabase and the user gets no signal; live repro on Tim's operation 2026-06-23)

**Status:** closed 2026-06-23 — surfacing + auto-retry shipped. `src/data/custom-sync.js` gains `getSyncHealth()` (combined push + DLQ snapshot — forces `status: 'failed'` whenever `getDeadLetters().length > 0`, promotes idle → syncing when the pending queue is non-empty), `getQueueLength()` (cheap accessor for the logout-confirm path that was already calling it), `_singlePush()` (single-shot push without the 1+2+4+8s backoff chain), and `_drainDeadLetters()` (bounded MAX_AUTO_DRAIN = 3 auto-retry over `flush()`). `flush()` now drains the DLQ after the main queue and reports `error` whenever the DLQ is still non-empty afterward. `src/ui/header.js` `getSyncState()` consumes `getSyncHealth()` — DLQ non-empty → `sync-failed` (red) with count label; `triggerManualPull()` first calls `adapter.retryDeadLetters()` + `adapter.flush()` whenever the indicator is red, then pulls. CSS adds `.sync-failed { background: var(--red); }` (distinct from `.sync-err` for the semantic distinction the spec asked for). i18n adds `sync.failed` ("Sync failed · {count} pending") and `sync.retryingFailed` ("Retrying failed syncs…"). 8 new tests in `tests/unit/sync-dead-letter.test.js` cover the health snapshot, the auto-drain heal path, the MAX_AUTO_DRAIN cap, the manual retry-via-tap path, and the queue-length accessor. Grep contract holds — `grep -rn "getDeadLetters\|retryDeadLetters" src/` shows `src/ui/header.js` calling `adapter.retryDeadLetters()` (the required outside-`custom-sync.js` consumer). Suite: 1529 → 1537 (+8), all green. V2_INFRASTRUCTURE.md §3.5 already documented the contract at filing time. Filed as GH-54.

**Bug.** `src/data/custom-sync.js` `_pushToRemote` retries a failed write `MAX_RETRIES` (5) times with exponential backoff, then calls `_deadLetter(...)`, which appends the record to a `_dead_letter_queue` in localStorage and logs one `logger.error('sync', 'Dead letter: …')`. After that the record is inert:

- `getDeadLetters()` and `retryDeadLetters()` exist but are **called from nowhere in `src/`** (grep confirms zero consumers). Nothing surfaces the dead-letter count and nothing retries it automatically.
- `_pushToRemote` sets status `'error'` on dead-letter, but the next successful push sets `'idle'` again, so the error state is transient and the OI-0141 sync indicator (`src/ui/header.js`) returns to green. The indicator reads `adapter.getStatus()` + `getLastPulledAt()` — neither reflects a non-empty dead-letter queue.

Net effect: a write that exhausts retries (a brief offline window is enough — see the 2026-06-17 `pullAll failed … TypeError: Failed to fetch` rows in `app_logs`) is dropped, the record stays only in localStorage, and the app shows a normal green-dot "synced" state. The only recovery is the manual full re-push `pushAllToSupabase()` (`src/data/push-all.js`, OI-0049 recovery path).

**Live repro (2026-06-23).** Tim corrected E-5's paddock-window open date in-app; the correction never reached Supabase (the row's `updated_at` stayed at its 2026-06-14 creation time until a direct-SQL repair). Two full re-pushes (`pushAllToSupabase`, 1148 records each) at 20:27 / 20:28 were what finally forced local state up — and one of those re-pushes overwrote a server-side correction, confirming the queue held un-synced local writes sitting silent.

**Fix (design).** Make failed syncs visible and self-healing:

1. **Auto-retry dead letters on reconnect.** On each successful `flush()` (and on `online` / `visibilitychange→visible`), drain the dead-letter queue back through `_pushToRemote` (bounded, with its own attempt ceiling) before declaring idle. A transient-failure record should heal itself on the next good connection without user action.
2. **Surface the dead-letter count in the sync indicator.** Extend the OI-0141 indicator: red (`sync-failed`) when `getDeadLetters().length > 0`, with the count; tapping it runs `retryDeadLetters()` + `flush()` and, if still failing, opens a short "couldn't sync N changes" detail. Green requires push queue empty AND dead-letter queue empty AND pull fresh.
3. **Stop the silent-green masking.** `getStatus()` (or a new `getSyncHealth()`) must factor in dead-letter and pending-queue length so a single later success can't repaint green over a real backlog.

**Files:** `src/data/custom-sync.js` (auto-retry hook in `flush`/reconnect, expose `getSyncHealth()`), `src/ui/header.js` (indicator consumes dead-letter count, tap → retry), `src/data/store.js` (wire reconnect/visibility retry trigger if not already present), `src/i18n/locales/en.json` (failed-sync label + count), tests.

**Acceptance criteria:**

- [ ] A push that exhausts `MAX_RETRIES` lands in the dead-letter queue AND the sync indicator turns red with the count (not green).
- [ ] On the next successful connection (reconnect / visibility / manual tap), dead letters auto-retry; on success the queue empties and the indicator returns to green with no user action beyond being online.
- [ ] Tapping the red indicator triggers `retryDeadLetters()` + `flush()`.
- [ ] `getStatus`/`getSyncHealth` cannot report a green/idle state while `getDeadLetters().length > 0` or the pending queue is non-empty.
- [ ] Grep contract: `grep -rn "getDeadLetters\|retryDeadLetters" src/` returns matches in `custom-sync.js` (defs) AND at least one consumer in `src/ui/` or `src/data/` — no longer zero consumers.
- [ ] Unit test: simulate 5 failed pushes → dead-lettered → health reports failed → reconnect → auto-retry succeeds → health green.
- [ ] V2_INFRASTRUCTURE.md sync section documents the dead-letter surfacing + auto-retry contract.

**CP-55/CP-56 spec impact:** None — sync-transport reliability only; no schema, column, JSONB, or migration change.

**Related:** OI-0141 (sync indicator push+pull semantics — this extends it with the failed/dead-letter dimension). OI-0049 (`pushAllToSupabase` recovery path — becomes the manual fallback, no longer the only recovery). OI-0050 / OI-0053 (silent-sync-failure class — same family: the app reads localStorage first, so a broken push is invisible).

**Origin:** 2026-06-23 cowork debugging session. Tim's E-5 window correction never synced; investigation of `custom-sync.js` found the dead-letter queue has no surfacing or auto-retry wiring.
...
**Status:** closed 2026-05-08 — A + B + C shipped in single bundled commit. `src/features/animals/index.js`: `renderGroupsList` switched to a `.groups-grid` 2-col CSS grid with `.groups-grid-tile` cards (no `×` delete affordance — Edit is the only path off an active group); `openGroupSheet` (now exported) carries the new `Cancel · Save · Archive · Delete` edit-mode footer wiring `archiveGroup(groupId)` from the store as the primary destructive path and `groupEventHistoryCount` as the Delete gate; `renderFilterHeader` drops the legacy `Show culled` checkbox + `showCulled` state variable, appends a `Culls (N)` chip with class `agc-chip-culls` (amber dot prefix, hidden when N === 0), and re-anchors `+ Add animal` as a single right-aligned button row below the search bar; `renderAnimalList` filter logic now keys off three mutually exclusive `selectedFilter` states (`null` / `'__culls'` / `<groupId>`). CSS additions to `src/styles/main.css`: `.groups-grid`, `.groups-grid-tile`, `.agc-chip-culls` (active + dot styles). 4 i18n keys added to `src/i18n/locales/en.json` (`animal.cullsChip`, `group.archiveButton`, `group.archivedToast`, `group.deleteDisabledTooltip`). 12 new tests in `tests/unit/animals/animals-area-ui-rework.test.js` cover the tile grid, the Archive/gated-Delete footer, and the Culls pill state machine. All five grep contracts hold (`Show culled` → 0, `showCulled` → 0, `×` at active-group tile → 0 [the 3 remaining matches are class/treatment/AI-bull CRUD list rows the contract explicitly excludes], `archiveGroup` in animals/index.js → 2, `groups-grid` ≥ 1 across `src/features/animals/` + `src/styles/`). Suite: 1517 → 1529 (+12), all green; lint + build clean. No schema, no migration, no CP-55/CP-56 impact (`groups.archived_at` already shipped in migration 024). Filed as GH-53; spec file renamed to `github/issues/GH-53_OI-0164_animals-area-ui-rework.md`.
...
- [ ] Live-repro regression test: simulate the 2026-05-06 D → B-3 sequence (live remaining = 0, default Move radio, click Save) — pre-OI-0162 path threw mid-execute and left a partial commit; post-OI-0162 path renders no Move/Residual radios (sub-item A), pre-flight succeeds, source closes cleanly, dest event written cleanly, wizard closes cleanly. Then simulate a SECOND wizard open on the now-closed source and assert sub-item C refuses to write.
...
**Spec file:** `github/issues/OI-0162_move-wizard-three-bugs.md` — full spec consolidated A/B/C, written same session.
...
**Resolution:** `src/features/events/move-wizard.js` Step 3 derives three state inputs at `renderStep3` entry — `mode` (`'scoped-remaining'` / `'scoped-last'` / `'full-event'`), `sourceLocationType`, `destLocationType` — and uses them to gate (a) the close-section title text and (b) which observation cards render. **Title gating:** `'scoped-remaining'` → `t('event.moveGroupOutOf', { group, paddock })`; `'scoped-last'` → `t('event.closePaddockNamed', { paddock })`; `'full-event'` → `t('event.closeSource')` (preserved). **Post-graze card** renders only when `(mode === 'full-event' || mode === 'scoped-last') && sourceLocationType === 'land'`. **Pre-graze card** renders only when `state.destType === 'new' && destLocationType === 'land'` (the existing-event / `'join'` path was already gated by the enclosing `if`). When not rendered, `postGraze` / `preGraze` stay `null` and the executeMoveWizard `if (postGraze)` / `if (preGraze)` guards (lines 574 / 578 / 724 / 798) skip validation + getValues — verified by walking the diff. **Defensive fallbacks:** `sourceLoc?.name ?? 'paddock'` and `sourceGroup?.name ?? 'group'` so a missing entity lookup degrades to "Move group" / "Close paddock" instead of crashing. **i18n:** two new keys in `src/i18n/locales/en.json`: `event.closePaddockNamed` ("Close {paddock}") and `event.moveGroupOutOf` ("Move {group} out of {paddock}"). **Surprise:** the spec's suggested key `event.closePaddock` collides with a long-standing literal `"Close paddock"` already at en.json:356 (used by `detail.js:503`'s per-paddock close button). Duplicate JSON keys are silently overridden by the LAST occurrence — the spec's suggested name was getting shadowed. Renamed to `closePaddockNamed` to disambiguate; comment in move-wizard.js documents why. **Three new data-testid attributes** for test selectors: `move-wizard-close-section-title` (close-section header), `move-wizard-post-graze-card` (postGraze.container), `move-wizard-pre-graze-card` (preGraze.container). **Tests:** new `tests/unit/move-wizard-step3-gating.test.js` (8 cases per spec): scoped+remaining+land → no post-graze, "Move Mixed Calves out of G - 5"; scoped+last+land → post-graze + "Close G - 5"; scoped+last+confinement → no post-graze + "Close Corral A"; full-event+land → post-graze + "Close Current Event"; full-event+confinement → no post-graze; land destination → pre-graze present; confinement destination → no pre-graze; join existing event → no pre-graze. All 8 pass post-fix. Suite 1489 → 1497 (+8). `npm run lint` 0 errors. `npm run build` clean. **Existing OI-0066 scoped-move tests** at `tests/unit/move-wizard-scoped.test.js` still pass — no regression on the close-out decision logic. **Schema impact:** none. **CP-55/CP-56 impact:** none — pure render-side gating. **Follow-up flagged for Cowork:** `V2_UX_FLOWS.md` §1 / §17.7 documentation already updated in the same Cowork session that drafted this OI; no Claude Code touch needed there.
...
**Resolution:** Two files, ~5 lines. (1) `src/styles/main.css` — three new design tokens added next to the existing `--color-amber-*` block: `--color-tan-base: #C9A875; --color-tan-dark: #8C7444; --color-tan-light: #F2E8D2;`. (2) `src/ui/dmi-chart.js` — `COLOR_STORED` swapped from `var(--color-amber-base)` → `var(--color-tan-base)`; `COLOR_STORED_STRIPED` second color swapped from the manually-picked `#E5C76B` (an amber-yellow sibling) → `var(--color-tan-light)` so the diagonal pattern stays in one tonal family. The legend swatch reads from `COLOR_STORED` so it picks up the new color automatically; nothing else in `dmi-chart.js` changes. **Other consumers of `--color-amber-base` (dashboard threshold colors, todo card, observation severity ramp, badges)** intentionally keep amber — amber is correct for *warning / pending* signals; tan is for *stored feed on data-viz surfaces* per V2_DESIGN_SYSTEM.md §1.1 "Tan vs. Amber". **Tests:** existing `tests/unit/dmi-chart.test.js` + `tests/unit/dmi-chart-context.test.js` (12 cases) green; the contract is unchanged (rendering color isn't asserted in tests). All three grep contracts hold: `var(--color-amber-base)` in `src/ui/dmi-chart.js` → 0 matches; `#E5C76B` in `src/ui/dmi-chart.js` → 0 matches; `color-tan-base` in `src/styles/main.css` → 1 match. Prior OI-0148 / OI-0149 / OI-0151 / OI-0152 / OI-0153 / OI-0154 / OI-0155 / OI-0157 / OI-0158 grep contracts all still hold. **Manual UI smoke test on Tim's populated op:** unverified by Claude Code; recommend cold-loading the deployed site and verifying the DMI bar's stored segment now reads visually distinct from the deficit-red segment on a deficit day. **Schema impact:** none. **CP-55/CP-56 impact:** none (presentation-layer only). **Doc updates** (already in place per the spec): `V2_DESIGN_SYSTEM.md` §1.1 + §5.4, `V2_UX_FLOWS.md` §17.15.
...
| 2026-05-08 | OI-0163 filed (P1) — Place button on unplaced groups dumps user on rotation calendar instead of opening a placement flow; both `Place` buttons in `src/features/dashboard/index.js` (line 920 group card + line 990 Unplaced groups section) run `() => navigate('#/events')` with no `openMoveWizard` call; V2_UX_FLOWS.md §17.7 lines 1259 + 1331 already documented the intended behavior so this is doc/code drift, not a regression | Tim split animals into new groups on 2026-05-07 and reported "the path takes me to the rotation calendar versus the wizard" when trying to place those new groups. Diagnosis: the dashboard handler at `src/features/dashboard/index.js:920` and `:990` literally navigates to `#/events` (the rotation calendar route per `main.js:266`). No existing OI for this exact bug; adjacent items (OI-0137 backdated cull unplacing groups; OI-0143 dashboard `.find()` returning stale windows; OI-0162 move-wizard triple bug from 2026-05-06) are different classes. Considered option 1 (one wizard with `sourceEvent === null` gating throughout) vs option 2 (separate `place-wizard.js` orchestrator on top of a new `wizard-shared.js`) and locked on option 2. Rationale: the move wizard already carries 5 orthogonal axes (scoped vs full-event, new vs join, source land vs confinement, dest land vs confinement, OI-0101 mirror touched vs not) and Step 3 alone has ~15 reads of source-side state; adding "source present vs absent" as a 6th axis on top of OI-0162's just-landed pre-flight + idempotency guards creates pressure to bypass those guards and makes the file read worse. Option 2 doesn't mean duplication — Step 1, Step 2, and `createDestinationEvent` / `joinExistingEvent` extract into `wizard-shared.js`; both `move-wizard.js` and `place-wizard.js` become thin orchestrators on top. Three sub-items locked: A — pure-refactor extraction of Step 1 + Step 2 + dest-creation into `wizard-shared.js`, no behavior change; B — new `place-wizard.js` reusing shared steps, Step 3 stripped to "Open destination" only (date in, time in, pre-graze card gated on `dest.type === 'land'`), save sequence is steps 6, 7, 9 of §1.6 only (skip 1, 2, 3, 4, 8 — all source-side work); C — wire dashboard Place buttons to `openPlaceWizard(group.id, operationId, farmId)` at both sites. Pre-open guard added: empty group (0 active memberships) → toast + abort, matches OI-0086 pattern. Cross-farm placement supported the same way as cross-farm Move (Step 2 farm chip overrides default); `source_event_id` stays null on placement. Field-mode currently has no Place action so no field-mode change required (Claude Code to confirm in implementation). Grep contracts locked: `grep -rn "navigate('#/events')" src/features/dashboard/index.js` returns 0 post-fix; `grep -n "sourceEvent\|sourceEventId\|source_event_id" src/features/events/place-wizard.js` returns 0 (seam enforced at file boundary, not by null checks); `grep -n "openPlaceWizard" src/features/dashboard/index.js` returns ≥ 2; `grep -n "function renderStep1\|function renderStep2" src/features/events/move-wizard.js` returns 0 after extraction. **Schema change:** NONE. **CP-55/CP-56 impact:** NONE — pure UI/wizard refactor; data shapes (events, paddock windows, group windows, observations, feed entries) unchanged. **V2_UX_FLOWS.md updates this session:** added §1.8 "Place Mode" subsection describing the sibling wizard, reused steps, Step 3 differences, save sequence; updated §17.7 line 1259 "Opens move wizard (§1)" → "Opens place wizard (§1.8)"; updated §17.7 line 1331 "opens move wizard with the group pre-selected and no source event" → "opens place wizard (§1.8) with the group pre-selected." **Spec file:** `github/issues/OI-0163_unplaced-place-flow.md` written same session, full A/B/C consolidated. **No code change this session — OPEN_ITEMS.md edits + V2_UX_FLOWS.md §1.8 + spec file only; ready for Claude Code in A → B → C order.** Related: OI-0162 (open, just-landed — the move-wizard pre-flight + idempotency guards that motivate the file-boundary split), OI-0086 (closed — empty-group toast pattern reused for Place pre-open guard), OI-0101 (closed — date/time mirror lives only on move wizard's Step 3, no equivalent in Place), OI-0114 (held — retro-place / gap-fill is a separate flow, not in OI-0163 scope), OI-0066 (closed — scoped-mode introduced the per-group Move button pattern; Place is the no-source sibling). |
| 2026-05-06 | OI-0162 filed (P1) — Move wizard three compounding bugs (A: 0-remaining still prompts move/residual; B: partial-write on validate throw with wizard left open; C: no idempotency guard on closed source); live repro 2026-05-06 D → B-3 produced an orphan destination event; Supabase cleanup applied same session | Tim moved Shenk Culls + Bull Group from D → B-3 with 0 live-remaining feed and reported "the form still prompts for whether to move it or leave as residual… first attempt blocked the move with an error… two B-3 events, one with the groups, one with no groups… on error it saved the event with the move wizard still open and appearing to have not completed." Supabase investigation confirmed a 46-second gap between two `events` rows targeting B-3 with the same `source_event_id`: Run 1 (09:54:29) wrote complete state — close-reading check (`5514413c…`), source D paddock window + 3 group windows closed, dest event `0d28b5ff…` with paddock window + 2 group windows (Bull + Shenk Culls) + open observation. Run 2 (09:55:15) wrote duplicate close-reading check (`dcbb820c…`), updated source D `date_out` to `2026-05-06 13:30`, created orphan dest event `01b1617f…` with paddock window + open observation but **zero group windows** (because `allOpenSourceGWs` was empty by then). **Smoking-gun validator** at `src/entities/event-feed-entry.js:46` (`if (typeof record.quantity !== 'number' || record.quantity <= 0) errors.push('quantity must be a positive number')`): with live-remaining = 0 and the default Move radio selected, Step 8's `add('eventFeedEntries', ...)` threw, the wizard's catch handler painted the error in `statusEl`, and `moveWizardSheet.close()` (last line of the try block) was never reached — the wizard stayed open showing the error AFTER source GWs/PW + dest event/PW/GWs/observation had already committed. Tim saw the error message, switched the radio to "Residual" with value 0, clicked Save again → second pass on a now-closed source → orphan event. Three bugs trace cleanly to specific lines: A = `move-wizard.js:417-538` (no early-exit on `group.total <= 0`), B = `move-wizard.js:566-853` (try without finally + non-transactional sequence + 280 lines of sequential writes inside a single try), C = `move-wizard.js:566` start (no source-state pre-check; wizard accepts re-run on closed event). **Locked design summarized:** A drops the (batch, location) group entirely from `feedGroups`/`transferToggles` when `total <= 0`, with a new "all-zero" hint when every group is zero (preserves OI-0135/OI-0139 invariants because Step 1 builds its own independent feedGroups Map). B adds two layers — Layer 1 hoists `FeedEntryEntity.validate` dry-run to pre-flight; Layer 2 wraps Steps 1-8 in `try/catch/finally` with `moveWizardSheet.close()` in the finally + a visible toast + `logger.error('move-wizard', ...)` so the failure lands in `app_logs` once OI-0150-C ships. C adds two refuse-to-act guards at the top of `executeMoveWizard` — refuses when `!isScoped && sourceEvent.dateOut` (full-event re-run on closed source), refuses when `sourceGWs.length === 0` (covers both scoped and full no-op cases). **Cleanup SQL applied same session:** orphan event `01b1617f-c54a-4048-8317-bbdbbb359cc1` + its paddock window `57257407-b178-4855-a1fe-3d5fc30e5f08` + the open observation `8167b0d0-5ce4-4218-8756-0acccf674626` deleted; duplicate close-reading check `dcbb820c-fec2-4a8f-8613-b9d03153f714` and its check item deleted; source D event/paddock window/group windows/close-reading aligned to `2026-05-05 13:00` per Tim's confirmation that the actual physical move happened on 2026-05-05. **Implementation order locked:** A → B → C; ship as a single bundle if possible (same file, same regression test). **Schema change:** NONE. **CP-55/CP-56 impact:** NONE. **PLUGIN IMPROVEMENT candidate flagged inline:** "Multi-step writers that span entity boundaries (move wizard, sub-move, advance-strip, future bulk operations) need transactional UX semantics — pre-flight every potentially-throwing condition, finally-close + visible toast on any unexpected throw, refuse-to-act on incoherent source state. Single-try-no-finally + paint-error-in-status is the worst of both worlds: user can't tell whether writes succeeded." Same family as OI-0050's writer-side blind spot but a different vector — there the bug was "writer writes nothing"; here it's "writer writes half." Worth a row in IMPROVEMENTS.md when OI-0162 lands. **No code change this session — OPEN_ITEMS.md edits + Supabase cleanup + spec file `github/issues/OI-0162_move-wizard-three-bugs.md` only; ready for Claude Code in A → B → C order.** Related: OI-0135 (closed — introduced `getLiveRemainingForMove`; Bug A extends the doctrine to "when helper returns 0, don't render the choice"), OI-0136 (closed — "Leave as residual" forced input; mooted for 0-remaining groups by Bug A since group no longer renders), OI-0139 (closed — same helper, no interaction), OI-0140 (closed — multi-window paddock disambiguation, no interaction), OI-0050 (closed — writer-side sync gap; Bug B extends the silent-failure doctrine to the wizard), OI-0090 / SP-11 (open — empty-group prompt; what initially made Tim think Run 1 had errored; not changed by this OI), OI-0117 / OI-0133 (closed — "two paths can disagree → consolidate to one truth"; Bug C's idempotency check is the same direction). |
