...
6. **Wholesale replace — transaction strategy.** Client-side per-table replace, in FK-dependency order per §5.3a, with halt-on-first-failure. For each table (iterating §5.3a top-to-bottom for deletes, then top-to-bottom again for inserts):
   - **Deletes:** iterate §5.3a **bottom-to-top**. For each table, `DELETE FROM {table} WHERE operation_id = $1` (for `operation_id`-scoped tables). Four join tables lack a direct `operation_id` column and must delete through their parent FK:
     - `todo_assignments` → `DELETE WHERE todo_id IN (SELECT id FROM todos WHERE operation_id = $1)`
     - `event_feed_check_items` → `DELETE WHERE feed_check_id IN (SELECT id FROM event_feed_checks WHERE operation_id = $1)`
     - `harvest_event_fields` → `DELETE WHERE harvest_event_id IN (SELECT id FROM harvest_events WHERE operation_id = $1)`
     - `survey_draft_entries` → `DELETE WHERE survey_id IN (SELECT id FROM surveys WHERE operation_id = $1)`
     When a new table is added without a direct `operation_id` column, add it to this list and to the `INDIRECT_OPERATION_TABLES` map in `backup-import.js`.
