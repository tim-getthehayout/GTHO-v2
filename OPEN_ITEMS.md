...
### OI-0145 — DMI-8 audit resolver: register the daily-breakdown calc as a card on the OI-0138 audit page; design call needed on rendering pattern (multi-day output → card shape)
**Added:** 2026-05-01 | **Area:** v2-build / dev-tools / audit / calcs | **Priority:** P3 (audit completeness — DMI-8 is registered in the calc registry but has no sidecar resolver yet, so it's the only originally-listed MVP calc that doesn't surface as a card on the OI-0138 audit page; not blocking field testing because the DMI bar chart already visualizes DMI-8 output)

**Status:** open — DESIGN REQUIRED, do not build until rendering pattern is locked.

**Origin:** OI-0138 Phase 5 ship-time deferral. Claude Code intentionally skipped DMI-8 from the resolver MVP and documented the deferral inline at `src/features/dev-mode/audit-resolvers.js` line 16: *"DMI-8 is high-fan-out and deferred to a follow-on commit; the audit will surface it automatically once the resolver lands here."* This OI is the tracked home for that deferral so it doesn't get lost.

**Why DMI-8 is different from DMI-2 / DMI-3 / FOR-1:**

DMI-8 is a daily breakdown — it produces **one output per day in the event's window**, with each day classified as one of:

- `actual` (real consumption measurement)
- `estimated` (computed from declining mass balance)
- `needs_check` (insufficient data — gates fired; renders grey on the bar chart)

For an event spanning seven days, DMI-8 produces seven outputs. The other MVP calcs (DMI-2, DMI-3, FOR-1) produce one output per call, which maps cleanly to a single calc card. DMI-8 doesn't.

**Design call needed — rendering pattern:**

Four candidate patterns, with trade-offs:

1. **Single calc card with a per-day table** — one card. Inside: a small table with date / state / value columns, one row per day. Compact; respects the "one card per calc" pattern; readable for short windows but unwieldy for 30-day winter-feeding events.
2. **Single summary card with state breakdown + click-to-expand** — one card. Shows aggregate counts: e.g. *"5 actual, 1 estimated, 1 needs_check"* + click-to-expand reveals the per-day table. Most useful for diagnosis ("you have 1 needs_check day — find it") at the cost of one extra click for full detail.
3. **One card per day** — for a 7-day event, 7 cards. Probably too many for long events; defeats the calc-cards section's "one card per calc" rhythm.
4. **No DMI-8 card at all** — the DMI bar chart already visualizes DMI-8's output. Make the deferral permanent and document that DMI-8 is bar-chart-only on the audit page. Trade-off: bar chart is visual-only; the calc card is what surfaces the *inputs with source trace*, which the bar chart doesn't. Diagnosing "why is day 3 grey?" still requires opening console without a card.

**Cowork lean:** Option 2 (summary card with click-to-expand). Maximum diagnostic value at minimal screen real-estate; the "click to expand" pattern is consistent with how child-record tables expand their rows. Renders well on both desktop and phone.

**Acceptance criteria (after design lock):**

- [ ] `resolveDmi8Inputs(eventId)` added to `src/features/dev-mode/audit-resolvers.js` with the locked rendering shape.
- [ ] `audit-resolvers.js` line 16 deferral comment removed.
- [ ] DMI-8 surfaces in the audit calc-cards section automatically (registry-driven iteration already in place per OI-0138 Phase 5).
- [ ] Resolver calls `getCalcByName('DMI-8').fn(...)` for the output — no formula re-implementation. Existing grep contract holds.
- [ ] Unit test for the resolver with a synthetic 7-day window covering all three states (actual / estimated / needs_check).
- [ ] e2e or visual verification: open the audit page on a real event with mixed-state DMI-8 days; confirm card renders the right counts/breakdown.

**Pre-conditions before opening a design session on this OI:**

- Inspect a real audit page on a 7+ day event to feel which rendering pattern fits the surrounding card density.
- Confirm whether OI-0144 lands first (CST-N, NPK-N cards added) — if it does, the cards section will be denser and Option 1's per-day table might be too much.

**CP-55/CP-56 impact:** none — pure audit-page enhancement, no schema or state-shape change.

**Reference points:**

- `src/features/dev-mode/audit-resolvers.js:16` — current deferral comment.
- `src/calcs/feed-forage.js:541` — DMI-8 registration in calc registry.
- `V2_CALCULATION_SPEC.md §4.2 DMI-8` — canonical spec for the cascade model.

**Next step:** Cowork + Tim walk the four rendering options on a real audit page, lock one, then write a thin Claude Code spec file pointing to this OI body.
