...
6. **Pure-insert flow invariants (OI-0115):** Some flows must NEVER mutate their parent entity — they're architecturally pure INSERTs. Protect these with grep-based pre-commit contracts:
   ...
   - **Multi-window paddock disambiguation (OI-0140):** Any feed/observation surface that picks a `location_id` from a multi-row collection must NOT use `[0]` against an unsorted localStorage-backed array. Sort by `(date_opened, time_opened)` descending (null `time_opened` → `00:00`) and take the most-recently-opened as the default; expose a picker when there's > 1 option so the user can override. The delivery sheet (`src/features/feed/delivery.js`) is the canonical implementation. Grep contracts:
     - `grep -n "activePWs\[0\]" src/features/feed/` — must return 0 matches. The pre-OI-0140 unsorted-index shortcut is gone; deliveries flow through the picker's `selectedLocationId`. Origin: OI-0140 (2026-05-01 — Tim's strip-graze G-event silently routed a bale physically placed at G-1 to G-3 because localStorage insertion order put G-3 at index 0).
...
8. **Sync-indicator semantics (OI-0141):** The sync indicator must report both push and pull state. Green (`sync-ok`) = push queue empty AND last pull within stale threshold (15 min). Amber (`sync-stale`) = push empty but pull older than threshold or null. Tapping the sync dot/strip triggers `pullAllRemote()`, not `navigate('#/settings')`. Grep contracts:
   - `grep -rn "navigate('#/settings')" src/ui/header.js` — must return 0 matches. The sync dot/strip onClick fires `triggerManualPull`, not Settings navigation.
   - `grep -rn "getLastPulledAt" src/` — must appear in `src/data/pull-remote.js` (definition) and `src/ui/header.js` (consumer). If a new sync indicator surface is added, it must consume `getLastPulledAt()` rather than only checking `adapter.getStatus()`.
