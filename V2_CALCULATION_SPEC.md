...
### 4.9 Survey/Forage Quality Domain (2 formulas)
...
**SUR-2: Bale-Ring Cover (`survey.baleRingCover`)**
- Computes: forage cover % for a paddock, derived from a count of bale-ring residue footprints. Used as an auto-fill helper inside the survey paddock card (V2_UX_FLOWS.md §7.4) — the user types a ring count, the calc fills `forageCoverPct`, the user can still override.
- Registered via `registerCalc({ id: 'survey.baleRingCover', … })` in `src/calc/survey-bale-ring.js`.
- Inputs:
  - `ringCount` — integer ≥ 0, the user-entered count of bale-ring footprints visible across the paddock
  - `ringDiameterFt` — numeric, default 12.0 (per `farm_settings.bale_ring_residue_diameter_cm` converted to feet at read-time; see V2_INFRASTRUCTURE.md §1.4 for the unit pattern, and note that `_cm` is the metric-internal storage column post-OI-0111 / migration 027)
  - `paddockAcres` — numeric, the paddock's area
- Output: `{ ringAreaSqFt, totalAreaSqFt, coverReducedPct, computedForageCoverPct }`
  - `ringAreaSqFt = π × (ringDiameterFt / 2)²` — the per-ring footprint (≈ 113.1 sq ft at 12 ft diameter)
  - `totalAreaSqFt = ringCount × ringAreaSqFt`
  - `paddockSqFt = paddockAcres × 43560` — only used when `paddockAcres > 0`
  - `coverReducedPct = min(100, (totalAreaSqFt / paddockSqFt) × 100)` — the share of the paddock the ring footprints cover
  - `computedForageCoverPct = max(0, round(100 − coverReducedPct))` — the auto-fill value written into the cover field
- **Model:** the calc treats bale-ring residues as the *bare* portion of the paddock — `100 − ringArea/paddockArea × 100` is the share that's forage. If field-testing reveals farmers think of rings as "where the forage *is*" instead of "where it *isn't*," only the `computedForageCoverPct` line flips; the stored `ringCount` doesn't change.
- **Paddock-area fallback:** when `paddockAcres` is null or zero, return `coverReducedPct = null` and `computedForageCoverPct = null`. The survey card surfaces this as *"↳ Set paddock acreage to estimate cover."* — the count is still saved on the observation, but no auto-fill happens.
- **Storage:** the *count* (`paddock_observations.bale_ring_residue_count` / `event_observations.bale_ring_residue_count`) is stored on the observation row, not the derived cover. The derived cover only flows into the cover field via the auto-fill — once written, it's persisted as `forage_cover_pct` exactly like a hand-entered value. Re-running the calc later (e.g. when the paddock's `acres` changes) does not retro-update prior observations; the historical observation reflects what the farmer saw on the day.
- **v2-only:** no v1 equivalent. The bale-ring helper is the one deliberate v2 addition to the otherwise v1-parity survey UX (V2_UX_FLOWS.md §7).
- **Origin:** SP-9 (Survey Sheet v1 parity, retired sprint spec). Closed during reconciliation Session D (2026-05-04).
...
|------|---------|---------|
| 2026-05-04 | Reconciliation Session D — SUR-2 bale-ring cover helper | New SUR-2 (`survey.baleRingCover`) added to §4.9 Survey/Forage Quality Domain. Documents the inputs (`ringCount`, `ringDiameterFt` defaulting to 12.0 from `farm_settings.bale_ring_residue_diameter_cm`, `paddockAcres`), the four-output shape (`ringAreaSqFt` / `totalAreaSqFt` / `coverReducedPct` / `computedForageCoverPct`), the model rationale (rings as bare ground; only the final cover-derivation line flips if domain intent inverts), the paddock-area fallback (null when `paddockAcres` is null/zero — the count is still saved on the observation, no auto-fill happens), and the storage rule (the count goes on `paddock_observations.bale_ring_residue_count` / `event_observations.bale_ring_residue_count`; the derived cover only flows in via auto-fill and is then persisted as a normal `forage_cover_pct` value — re-running the calc later does NOT retro-update prior observations). v2-only formula (no v1 equivalent). Total formulas: 39 → 40. Pairs with V2_UX_FLOWS.md §7.4 (the user-facing surface) and the retired `github/issues/GH-12_survey-sheet-v1-parity.md` (now a thin pointer). Origin: SP-9 (retired sprint spec). No code changes — documentation catch-up only. Owner: Cowork. |
