...
**Status:** open — **Tim chose Option A (drop)** on 2026-04-20 conditional on a pre-spec audit confirming no live readers remained. Cowork ran the audit 2026-04-20 afternoon. Findings: zero live readers across `src/`, `src/tests/` is clean except historical negative-assertion test docstrings, one pre-OI-0112 orphan row in Supabase on Tim's operation (parent event's pre-graze data is intact in `paddock_observations` — CASCADE handles it), three stale pointer-comments in non-deleted files retargeted. Spec ready at `github/issues/drop-event-observations-table.md` with the audit table inlined.
