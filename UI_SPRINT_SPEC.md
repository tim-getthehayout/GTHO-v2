...
- [ ] Convert `github/issues/` files to thin pointers referencing base doc sections — pending; base-doc destinations now exist for the SP-3 / SP-8 / SP-10 / SP-12 work (Session B). Sweep can run in Session C alongside the remaining SP-6 / SP-7 / SP-9 / SP-13 / SP-14 thin-pointer conversions.
...
- [x] **SP-11 schema reconciliation** — ~~replace `archived boolean` with `archived_at timestamptz` in §3.3~~ **done 2026-05-03 (RECONCILIATION_PLAN_2026-05-03 SCH-1).** UX portion (§3.4 "Empty Group Handling" + §15.2 Group CRUD Sheet "Show archived / Reactivate") **pending** — Session C. V2_MIGRATION_PLAN.md §5.3a — no FK change needed. CP-55/CP-56 spec updates with `archived_at` column handling **subsumed by OI-0156**. `github/issues/empty-group-archive-flow.md` thin-pointer conversion **pending** — happens with the UX merge.
