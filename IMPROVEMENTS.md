...
### 15. `git add` atomicity + pre-rename `github/issues/` pathspec strands sibling docs
**Plugin:** project-infrastructure
**Skill:** deploy-gate, doc-workflow
**What:** When Cowork provides a paste-ready `git add A B C D` command where one of the args is a `github/issues/*.md` spec file that Claude Code is expected to `gh issue create` + rename (to `GH-N_*.md`) during its implementation session, the combined command has a silent-failure mode: if the rename happens before the `git add` runs (e.g. Claude Code processes the spec first), the pathspec on the pre-rename filename fails, `git add` aborts **atomically** — meaning A, B, and C are never staged either — and the subsequent retry almost always drops one of the sibling files from the list by hand. The deploy-gate should generate commit commands that isolate `github/issues/*.md` files so a failure on that one path does not orphan sibling doc changes.
**Why:** Hit on GTHO-v2 OI-0119 (2026-04-20). Cowork wrote the spec to `github/issues/dmi-8-cascade-rewrite.md` and provided a push command `git add OPEN_ITEMS.md V2_CALCULATION_SPEC.md UI_SPRINT_SPEC.md github/issues/dmi-8-cascade-rewrite.md`. Claude Code's next session ran `gh issue create` and renamed the file to `GH-29_dmi-8-cascade-rewrite.md` before Tim ran the add. The add failed with `fatal: pathspec 'github/issues/dmi-8-cascade-rewrite.md' did not match any files` — nothing staged. On retry the three base docs were re-added (against the now-present `GH-29_` name for the fourth), commit ca332c0 landed, but `UI_SPRINT_SPEC.md` got dropped from the retry list and sat uncommitted for ~3 hours while Claude Code went on to implement + ship the feature from the already-pushed pieces. The stranded doc had stale filename references (`dmi-8-cascade-rewrite.md` × 3) that had to be patched in a cleanup session. Same failure class is latent on every spec-handoff session that uses pre-rename filenames in the push command. The existing verify-push-landed rule (IMPROVEMENT #14) does not catch this — the push of the partial set genuinely lands on origin, so the SHA-match check passes. The bug is in what was staged, not what was pushed.
**How to apply:**
1. `doc-workflow` SKILL.md — add a rule to the "Spec Handoff to Claude Code" section: *When a Cowork session produces a new spec file in `github/issues/` without a `GH-N_` prefix, do not include that file in the Cowork-provided git add/commit command at all. Claude Code will add + commit the file when it creates the GitHub issue (which also handles the rename). Cowork's push command should only stage the design docs and OPEN_ITEMS.md.* This eliminates the race entirely.
2. `deploy-gate` SKILL.md — in the paste-ready git-push command generator: if the changed-files set includes any `github/issues/*.md` without a `GH-N_` prefix, split the command into two commits — one for the base docs (atomic), one for the spec file (isolated) — so a pathspec failure on the spec does not orphan the base-doc changes. Alternative form: one `git add` per file, each on its own line, so a failure is visible and scoped.
3. `deploy-gate` SKILL.md — add a post-commit, post-push check that reads `git status --porcelain` and flags any staged-or-modified files not in the commit. Catches the "retry dropped a file" class even without knowing about the rename.
**Where:** doc-workflow SKILL.md §Spec Handoff; deploy-gate SKILL.md §Paste-Ready Commands and §Cowork Delivery Gate.

**Related memory:** user auto-memory `feedback_push_and_prompt.md` (always provide push + CC prompt) and `feedback_verify_push_landed.md` (verify SHA on origin). This IMPROVEMENT closes the gap both miss: the push lands, the SHA matches, but the wrong set of files was in it.

### 16. OPEN_ITEMS.md closure discipline — siblings, orphans, and downstream-moot entries
**Plugin:** project-infrastructure
**Skill:** deploy-gate, doc-workflow
**What:** OPEN_ITEMS.md drifts stale because three distinct failure modes in the close-out process go unnoticed. Add three rules to make each failure mode visible at the moment it happens, not weeks later during a reconciliation sweep:

1. **Piggyback rule.** When closing an OI whose fix lands alongside a larger headline change (one commit closes multiple OIs because the underlying code path is shared), grep OPEN_ITEMS.md for every sibling OI that touches the same file or feature before committing. Flip all affected status lines in the same commit. Don't assume the headline OI is the only one the change closed.
2. **Orphan-flip belt-and-braces.** Any commit whose message cites `OI-NNNN` must also include a staged edit to OPEN_ITEMS.md in the same commit. Pre-commit grep contract: if `git log -1 --format=%B` matches `OI-\d+` then `git diff-tree --no-commit-id --name-only -r HEAD | grep OPEN_ITEMS.md` must return a match. Prevents the "fixed the code but never flipped the status line" class.
3. **Downstream-moot sweep.** When a structural change (table drop, column rename, file delete, schema_version bump, migration rule added to `backup-migrations.js`) retires a symptom that an older OI was tracking, that OI is now moot. Before committing the structural change, grep OPEN_ITEMS.md for the retired symbol or the OI ID, flip any matches to closed with a "made moot by OI-NNNN / migration NNN" note in the same commit. The person shipping the downstream change is the only one positioned to know the older OI became moot — a later reconciliation pass can only find it by accident.

**Why:** The 2026-04-20 reconciliation sweep flipped five OIs stale by a week or more — OI-0116, OI-0087, OI-0088, OI-0052, OI-0051. Each matched one of the three patterns: OI-0116 piggybacked on OI-0117 (sibling); OI-0052 piggybacked on the main.js auth rewrite (sibling); OI-0051 shipped cleanly but the status line just wasn't flipped (orphan); OI-0087 and OI-0088 became moot when OI-0113 dropped `event_observations` and migration 029 bumped schema_version to 29 (downstream-moot). All five would have shown up weeks earlier with less diagnostic cost if the three rules above had been enforced at close-out time. The Cowork Delivery Gate today has one "OPEN_ITEMS.md changes saved and pushed" step — good but insufficient: it doesn't prompt the author to consider siblings or downstream-moot entries.
**How to apply:**
1. `deploy-gate` SKILL.md — Cowork Delivery Gate checklist gains a "sibling-OI audit" step immediately after any OI-closing edit: *"Before staging the OPEN_ITEMS.md edit, grep for every OI that references the same file path, feature name, or symbol as the OI being closed. Flip all that are now resolved in the same commit — don't leave siblings to a later reconciliation pass."* Also add a "downstream-moot sweep" step for structural changes: *"When a commit drops a table, renames a column, deletes a file, bumps `schema_version`, or adds a `BACKUP_MIGRATIONS` rule, grep OPEN_ITEMS.md for the retired symbol and flip any now-moot entries to closed in the same commit."*
2. `deploy-gate` SKILL.md — add a post-commit grep contract: *"If the commit message cites `OI-NNNN`, verify `git diff-tree --no-commit-id --name-only -r HEAD | grep OPEN_ITEMS.md` returns a match. If it doesn't, the status line wasn't flipped — amend or follow with a corrective commit in the same session."*
3. `doc-workflow` SKILL.md — spec-handoff section adds a "passenger OIs" note: *"When writing a spec file or session brief that targets a headline OI, list every sibling OI expected to close as a passenger so Claude Code has a complete set of status-line edits to make in the implementation commit."* Template line for session briefs: `Closes: OI-NNNN (headline) + OI-MMMM, OI-KKKK (passengers — same code path).`
4. Optional complement in a pre-commit hook (project-scaffold): a hook that greps the commit message for `OI-\d+` and warns if OPEN_ITEMS.md is not staged. Project-specific; the skill-level rules above are the portable part.
**Where:** `deploy-gate` SKILL.md (sibling-OI audit + downstream-moot sweep in Cowork Delivery Gate; post-commit grep contract after any OI-citing commit). `doc-workflow` SKILL.md (Spec Handoff section — passenger-OI listing template). `project-scaffold` SKILL.md (optional pre-commit hook for OI-cite / OPEN_ITEMS.md staging coherence).

**Related memory:** builds on `feedback_push_and_prompt.md` (provide push command after doc edits) and IMPROVEMENT #14/#15 (verify-push-landed, atomic git add) — same family of silent-drift failures in the Cowork → Claude Code handoff. This entry closes the gap on OPEN_ITEMS.md status drift specifically.

### 17. Consumer-sweep on calc/helper fixes — when fixing a formula, grep for *other* code that computes the same value
**Plugin:** project-infrastructure
**Skill:** deploy-gate, doc-workflow
**What:** When a fix lands on a calculation, helper, or derivation, the close-out checklist must include a "consumer sweep" that greps the codebase for any other place computing the same value by hand. If a duplicate formula is found, either fix it in the same commit or consolidate both consumers behind a shared helper before closing the OI. This is the same logic as the existing OPEN_ITEMS.md piggyback rule (IMPROVEMENT #16) but one layer deeper — into the *code surface* rather than the *OI surface*. Piggyback asks "are there sibling OIs?"; consumer-sweep asks "are there sibling code paths computing the same thing?"

**Why:** OI-0139 (2026-04-30) — the move-wizard live-remaining formula was fixed in OI-0135 eight days earlier (2026-04-22). The identical formula sat duplicated in `src/features/feed/check.js` and silently regressed Tim's Pasture D feed-check sheet — a fresh bale delivered after a 0-remaining check disappeared from the next prefill. The fix would have been one commit instead of two, eight days apart, if OI-0135's close-out had included "grep for any other code computing live-remaining per (batch, location)." The duplicate formula was findable by a single `grep -rn "lastCheckUnits != null"` on the day OI-0135 shipped. This is the same class as the OPEN_ITEMS.md closure-discipline rules but expressed at the source-code layer: *a fix lands on one consumer, but other consumers of the same calculation exist and silently retain the bug*. Especially load-bearing for compute-on-read derivations (the GTHO v2 architecture has many — DMI cascades, live-remaining feed, group current farm, event start, etc.) where a duplicated formula is a latent drift point indistinguishable from the consolidated case until field data exercises both.

**How to apply:**
1. `deploy-gate` SKILL.md — Cowork Delivery Gate gains a "consumer sweep" step paired with the existing piggyback step: *"When closing an OI that fixes a calculation, helper, or derivation, grep `src/` for the symbol or formula being fixed. If duplicate implementations exist, either fix them in the same commit or refactor to consolidate behind a shared helper. Flag any remaining duplicates in OPEN_ITEMS.md as a follow-on OI before closing the headline."* Concrete grep targets: function names being changed, variable names that encode the formula intent (`liveRemaining`, `pasturePool`, etc.), and key formula tokens (`lastCheckUnits != null`, `>= dateOpened`, etc.).
2. `doc-workflow` SKILL.md — spec-handoff section: when writing a fix spec for a calc/helper, the "Files to edit" section should be preceded by a "Consumer sweep" subsection listing every consumer the helper is known to have, so Claude Code's implementation pass covers them all up front. Example for OI-0139's spec: *"Consumers of `getLiveRemainingForMove`: move-wizard Step 3 (move-wizard.js:680), sub-move close hint (submove.js), feed-check sheet prefill (feed/check.js:84-87 — currently duplicated, consolidate in this commit)."*
3. CLAUDE.md template — extend the "Architecture Audit — Before Every Commit" section with an "Invariants on shared formulas" item: *"For any compute-on-read derivation that has more than one consumer, list the consumers and the symbol/helper they all must call. Add a grep contract that fails if any consumer drifts back to a hand-rolled formula."* OI-0139 added exactly this for live-remaining (CLAUDE.md §"Architecture Audit" §6 — three grep contracts: no `lastCheckUnits != null ?` in `feed/check.js`, strict-`>` present in `feed-state.js`, every consumer surface imports `getLiveRemainingForMove`). That pattern is the deliverable shape for any consolidated derivation going forward.

**Where:** `deploy-gate` SKILL.md (Cowork Delivery Gate — add consumer-sweep step alongside piggyback). `doc-workflow` SKILL.md (Spec Handoff — Consumer sweep subsection in fix specs targeting calcs/helpers). `project-scaffold` SKILL.md / CLAUDE.md template (Architecture Audit — "Invariants on shared formulas" item with the OI-0139 three-grep-contract pattern as the canonical example).
