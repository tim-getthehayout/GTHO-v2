...
export const FIELDS = {
  ...
  baleRingResidueDiameterFt:   { type: 'numeric',     required: false, sbColumn: 'bale_ring_residue_diameter_ft' },
...
export function create(data = {}) {
  return {
    ...
    baleRingResidueDiameterFt: data.baleRingResidueDiameterFt ?? null,
...
export function toSupabaseShape(record) {
  return {
    ...
    bale_ring_residue_diameter_ft: record.baleRingResidueDiameterFt,
...
export function fromSupabaseShape(row) {
  return {
    ...
    defaultAuWeightKg: row.default_au_weight_kg,
    defaultResidualHeightCm: row.default_residual_height_cm,
    defaultUtilizationPct: row.default_utilization_pct,
    recoveryRequired: row.recovery_required,
    defaultRecoveryMinDays: row.default_recovery_min_days,
    defaultRecoveryMaxDays: row.default_recovery_max_days,
    nPricePerKg: row.n_price_per_kg,
    pPricePerKg: row.p_price_per_kg,
    kPricePerKg: row.k_price_per_kg,
    defaultManureRateKgPerDay: row.default_manure_rate_kg_per_day,
    feedDayGoal: row.feed_day_goal,
    forageQualityScaleMin: row.forage_quality_scale_min,
    forageQualityScaleMax: row.forage_quality_scale_max,
    baleRingResidueDiameterFt: row.bale_ring_residue_diameter_ft != null ? Number(row.bale_ring_residue_diameter_ft) : null,
    thresholdAudTargetPct: row.threshold_aud_target_pct,
    thresholdAudWarnPct: row.threshold_aud_warn_pct,
    thresholdRotationTargetPct: row.threshold_rotation_target_pct,
    thresholdRotationWarnPct: row.threshold_rotation_warn_pct,
    thresholdNpkWarnPerHa: row.threshold_npk_warn_per_ha,
    thresholdCostPerDayTarget: row.threshold_cost_per_day_target,
    thresholdCostPerDayWarn: row.threshold_cost_per_day_warn,
