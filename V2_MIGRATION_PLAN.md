...
**Global reference tables** (`dose_units`, `input_product_units`) have no `operation_id` column — they are shared across all operations (Design Principle #8 exemption). On export, they are fetched without an `operation_id` filter. On import, they merge by `id` (upsert) — backup rows overwrite any existing row with the same `id`; rows not in the backup are left as-is. They are never deleted during import.

**Per-operation seed tables** (`treatment_categories`, `treatment_types`, `input_product_categories`, `forage_types`, `animal_classes`) have `operation_id` and follow the normal delete-then-insert path like all other operation-scoped tables. They are included in exports because users extend them beyond the default seed data.
...
**Note on `event_observations` removal (2026-04-20, OI-0113, migration 029).** The table was dropped after OI-0112 migrated every writer and OI-0119 migrated the last reader (DMI-8) to `paddock_observations`. Prior revisions of this list had `event_observations` at position 32 with `event_paddock_windows` as its parent; the drop removed that entry and renumbered positions 33–50 up by one. CP-56's 28→29 migration rule discards the key from older backups (`src/data/backup-migrations.js` entry 28).
...
6. **Wholesale replace — transaction strategy.** Client-side per-table replace, in FK-dependency order per §5.3a, with halt-on-first-failure. For each table (iterating §5.3a top-to-bottom for deletes, then top-to-bottom again for inserts):
   ...
   - **Global reference tables** (`dose_units`, `input_product_units`) have no `operation_id` column. Skip them during the delete phase. During the insert phase, `UPSERT` by `id` — backup rows overwrite matching rows; rows not in the backup are left as-is. When fetching these tables (for auto-backup or parity check), SELECT without an `operation_id` filter.
