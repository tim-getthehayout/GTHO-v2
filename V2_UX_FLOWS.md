...
### 15.0 Animals Screen Layout

The animals route (`#/animals`, rendered by `src/features/animals/index.js`) is the screen-level container that hosts the §15.1 Animal Edit Sheet and the §15.2 Group CRUD Sheet. It owns the cross-cutting filter UI for the animals list and the groups overview that links into both sheets.

**Top-level layout (top → bottom):**

1. **Filter chip row.** A single horizontally-scrollable strip (`.agc-chips`) of pills:
   - **`All`** — default selected. Animals list shows active animals only.
   - **Per-group chips** — one per active (non-archived) group, with the group's colour as a left-side dot. Tapping a group chip filters the animals list to active members of that group. Tapping the active group chip again deselects → returns to `All`.
   - **`Culls (N)`** — appended at the end of the chip row when the operation has ≥ 1 culled animal (`active === false`). Visual: amber dot prefix (`var(--amber)`) for parity with the legacy Show-culled accent. Tapping switches the animals list to **culls only** (and deselects whatever group chip was active). Hidden when `N === 0`.
   - The three families of chip — `All`, group, `Culls` — are mutually exclusive. Selection state is held in a single `selectedFilter` variable: `null` for `All`, the group ID for a group chip, or the sentinel string `'__culls'` for the `Culls` chip.
2. **Search bar.** Full-width text input below the chips. Filters by tag, EID, name, or class within whatever lifecycle state is currently active — i.e. when `Culls` is selected, the search filters within culls.
3. **Add-animal action.** `+ Add animal` button right-anchored. Opens §15.1 in create mode.
4. **Config row.** Three small outline buttons: `🐄 Classes`, `💉 Treatments`, `🐂 AI Sires`. Each opens its respective management sheet (animal classes, treatment library, AI sire library).
5. **Groups section.** Header row (`Groups` title, `Show archived (N)` toggle when archived groups exist, `+ Add group` button), followed by the **active groups grid** (see below). Below the active grid, a single-column **archived groups section** renders only when the Show-archived toggle is on.
6. **Animals list.** Sortable table (Tag/ID, Class, Group, Weight columns) of the filtered animal set, with per-row quick-action bar (§14.10).

**Active groups grid (the primary surface this section governs).**

The active groups list renders as a **2-column CSS grid** at all viewports (≥ 320px), not just desktop. `grid-template-columns: 1fr 1fr; gap: 8px; align-items: stretch;`. Tim's design intent is to recover the wasted horizontal space the legacy single-column row layout left on phone.

Each tile is a self-contained card with:

- **3px left colour stripe** in the group's `color` (default `var(--green)`).
- **Title row** — group name (14px / 600) + status badge (`badge bt` with text `active · {locationName}` when the group has an open `event_group_window` joined to an open event with at least one open paddock window; `badge bb` with text `unplaced` otherwise).
- **Sex breakdown line** — `{N} female, {M} male` (12px secondary), suppressed when both counts are zero.
- **Metrics line** — `{headCount} head · avg {avgWeight} {unit} · DMI target {dmiKg} {unit}/day`. Average weight is computed over the group's active memberships using each animal's latest `animal_weight_records` row (falling back to `animal_classes.default_weight_kg` when no record exists). DMI target is `totalWeightKg × 0.025`. Both display in the user's unit preference.
- **Action button row** — `Edit` · `Split` (only when the group is placed) · `Weights`, all `btn btn-outline btn-xs`. The button row may wrap on narrow phones (380px ÷ 2 ≈ 190px per tile); wrapping is preferred to label truncation.
- **Filtered state highlight** — when the user has tapped this group's chip in the filter row, the tile's background switches to `var(--green-l)` and its border-radius adjusts to match.

The tile's whole surface (excluding the action buttons, which `stopPropagation`) is clickable: tapping the tile body is equivalent to tapping the group's filter chip. There is **no `×` delete affordance** on an active group tile — destruction goes through Edit → Archive (default) or Edit → Delete (gated; see §15.2 footer actions).

**Archived groups section.**

When the Show-archived toggle is on, an `Archived groups` subhead renders below the active grid, followed by single-column rows (one per archived group). Each row shows: group name + `archived` badge, archive date + last headcount + historical event count summary, and two actions:
- **Reactivate** (`btn btn-green btn-xs`) — clears `archived_at` so the group reappears in the active grid.
- **Delete** (red outline) — disabled with tooltip *"This group is on N event(s). Archive instead to preserve history."* when the group has ≥ 1 `event_group_windows`. Enabled and one-step destructive otherwise.

The archived section stays single-column because the per-row text is longer (date + headcount + event count) and the action buttons are wider than the active tile's outline buttons.

**Origin (2026-05-08, OI-0164):** the 2-column grid + Culls pill replaced (a) a single-column row layout that wasted horizontal space on phone and (b) a `Show culled` checkbox that mixed culls into the active list at 50% opacity (hard to find one cull among hundreds).
...
**Edit-mode footer actions (OI-0164, 2026-05-08).**

The Group Edit sheet's footer carries four buttons — `Cancel` · `Save` · `Archive` · `Delete` — with Archive as the primary destructive action and Delete as a gated fallback. Create mode hides Archive and Delete; only Cancel and Save render.

| Button | Class | Visibility | Action |
|--------|-------|------------|--------|
| Cancel | `btn btn-outline` | Always | Closes the sheet without writing. |
| Save | `btn btn-green` | Always | Persists field edits (name, color, member changes). |
| Archive | `btn btn-amber` | Edit mode only | Stamps `archivedAt = now()`, queues sync, closes the sheet, fires toast `Group {name} archived`. Same write path as the empty-group prompt's Archive button (§3.4). The group disappears from the active grid; it can be brought back via Reactivate in the archived-groups section (§15.0). |
| Delete | red outline (`btn btn-outline` with `--red` color) | Edit mode only; **disabled when the group has ≥ 1 `event_group_windows`** | Disabled state shows tooltip *"This group is on N event(s). Archive instead to preserve history."*. Enabled state is one-step destructive (`confirm("Delete group {name}?")`) and removes the row outright. |

The archive-first hierarchy mirrors the archived-groups row controls in §15.0 (same gate, same wording). It exists because group provenance is load-bearing for event history (`event_group_windows.groupId` is the only thing that connects a closed event to which group was on the paddock). Deleting a group with event history would leave dangling references and no way to render historical event detail correctly.

There is no separate `×` delete affordance on the active groups grid — Edit is the single entry point for archiving or removing a group.
...
| 2026-05-08 | Animals area UI rework (OI-0164) | **NEW §15.0 Animals Screen Layout** documents the screen-level container that hosts §15.1 + §15.2 — chip filter row (`All` + per-group + new `Culls (N)` pill, mutually exclusive via `selectedFilter` sentinel `'__culls'`), search bar, `+ Add animal`, config row (Classes / Treatments / AI Sires), active groups grid, archived groups section. The active groups list switches from a single-column row layout to a **2-column CSS grid at all viewports** (`grid-template-columns: 1fr 1fr; gap: 8px; align-items: stretch;`) — Tim's design intent is to recover wasted phone-width space. Each tile carries 3px left colour stripe, name + status badge, sex-breakdown line, head/avg-weight/DMI line, and Edit / Split (when placed) / Weights buttons; the `×` delete affordance on active group rows is removed (destruction now goes through Edit → Archive or Edit → Delete only). Archived groups stay single-column. The legacy `Show culled` checkbox is dropped; culls are reached only through the new `Culls (N)` chip, which switches the animals list to **culls only** (exclusive view, not the dimmed-mix that came before). The 0.5-opacity styling on culled rows persists as the visual confirmation inside the Culls view. **§15.2 Group CRUD Sheet** gains an `Edit-mode footer actions` subsection: `Cancel · Save · Archive · Delete`, with Archive as the primary destructive action (stamps `archivedAt = now()` via the same path as the §3.4 empty-group prompt) and Delete gated on `event_group_windows` count (disabled with the *"This group is on N event(s). Archive instead to preserve history."* tooltip when ≥ 1; enabled one-step destructive otherwise). No schema, entity, JSONB, migration, or CP-55/CP-56 impact — pure UI rework over existing `groups.archived_at` (migration 024). Owner: Cowork. |
