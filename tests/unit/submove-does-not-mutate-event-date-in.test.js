...
import { getEventStartDate } from '../../src/features/events/event-start.js';
...
function seed() {
  ...
  // setEventStart prompts via window.confirm when tied-earliest windows move
  // together; auto-accept in tests.
  ...
  // window's date_opened is the source of truth for the event start.
...
describe('OI-0115 — sub-move Open must not mutate the derived event start', () => {
  it("baseline repro (Tim's exact flow): derived start stays unchanged after sub-move Save", () => {
    ...
    // OI-0117: "event.date_in stable" becomes "getEventStartDate stable" +
    // "anchor paddock window's date_opened stable".
    expect(getEventStartDate(EVT)).toBe(ORIG_DATE_IN);
    expect(getById('eventPaddockWindows', PW_G1).dateOpened).toBe(ORIG_DATE_IN);
  ...
  it('with Event Detail open: sub-move Save still does not mutate the derived start (suspect 1 scenario)', () => {
  ...
  it("phantom change on dateInInput (simulating iOS Safari's teardown native-picker cascade) is a no-op", () => {
    ...
    // the input's .value. The handler must NOT write through the phantom.
    dateInInput.value = SUBMOVE_DATE;
    dateInInput.dispatchEvent(new Event('change', { bubbles: true }));

    // Pre-fix this would have overwritten the derived start.
    expect(getEventStartDate(EVT)).toBe(ORIG_DATE_IN);
    expect(getById('eventPaddockWindows', PW_G1).dateOpened).toBe(ORIG_DATE_IN);
    closeEventDetailSheet();
  });
  ...
  it('genuine user edit writes through to the earliest child paddock window (fix does not break the legitimate path)', async () => {
    openEventDetailSheet({ id: EVT }, OP, FARM);
    const hero = document.querySelector('[data-testid="detail-summary"]');
    const dateInInput = hero.querySelector('input[type="date"]');
    expect(dateInInput.value).toBe(ORIG_DATE_IN);

    // Real user edit: connected input, differs from render-time snapshot,
    // no later siblings would be orphaned.
    dateInInput.value = '2026-04-15';
    dateInInput.dispatchEvent(new Event('change', { bubbles: true }));
    // setEventStart is async — flush microtasks.
    await Promise.resolve(); await Promise.resolve();

    expect(getEventStartDate(EVT)).toBe('2026-04-15');
    expect(getById('eventPaddockWindows', PW_G1).dateOpened).toBe('2026-04-15');
    closeEventDetailSheet();
  });
});

describe('OI-0115 — adjacent-flow smoke: Sub-move Close Save', () => {
  it('sub-move Close Save does not mutate the derived event start', () => {
    add('eventPaddockWindows', PaddockWindowEntity.create({
